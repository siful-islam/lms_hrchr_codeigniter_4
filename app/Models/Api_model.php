...
class Api_model extends NativeBaseModel
{
	...
	public function top_courses_get($top_course_id = "")
	{
		...
		$top_courses = $this->db->table('course')->get()->getResultArray();
	...
	function all_categories_get(){
		...
		foreach ($categories as $key => $category) {
			...
			$sub_categories = $this->db->table('category')->get()->getResultArray();
	...
	public function categories_get($category_id)
	{
		...
		foreach ($categories as $key => $category) {
			...
			$categories[$key]['number_of_courses'] = $this->crud_model->get_category_wise_courses($category['id'])->getNumRows();

			$this->db->where('parent', $category['id']);
			$number_of_sub_categories = $this->db->table('category')->get()->getNumRows();
	...
	public function sub_categories_get($parent_category_id)
	{
		...
		foreach ($categories as $key => $category) {
			...
			$number_of_courses = $this->db->table('course')->get()->getNumRows();
	...
	{
		$category_details = $this->crud_model->get_category_details_by_id($category_id)->getRowArray();
	...
	function filter_course()
	{
		...
		if ($selected_category != "all") {
			$category_details = $this->crud_model->get_category_details_by_id($selected_category)->getRowArray();
		...
		foreach ($courses as $course) {
			if ($selected_rating != "all") {
				$total_rating =  $this->crud_model->get_ratings('course', $course['id'], true)->getRow()->rating;
				$number_of_ratings = $this->crud_model->get_ratings('course', $course['id'])->getNumRows();
	...
	public function course_data($courses = array())
	{
		foreach ($courses as $key => $course) {
			...
			$instructor_details = $this->user_model->get_all_user($course['user_id'])->getRowArray();
			...
			$courses[$key]['total_enrollment'] = $this->crud_model->enrol_history($course['id'])->getNumRows();
	...
	{
		$query = $this->db->table('settings')->get()->getResultArray();
    ...
    function new_login_confirmation($param1 = "", $user_id = ""){
        ...
        if($param1 == 'submit'){
            $new_device_verification_code = $this->request->getPost('new_device_verification_code');
        ...
        if($validity === 'unverified_user' || $validity == true) {
            ...
            if (get_settings('student_email_verification') == 'enable') {
            	 if($validity === 'unverified_user'){
	    			...
	    			$query = $this->db->table('users')->where($credentials)->get();
	    			$this->email_model->send_email_verification_mail($data['email'], $query->getRow()->verification_code);
	...
	public function verify_email_address_post(){
	    ...
	    $query = $this->db->table('users')->where($credentials)->get();
	    if($query->getNumRows() > 0){
	      $this->db->where('id', $query->getRow()->id);
	      $this->db->table('users')->update(array('status' => 1));
	...
	public function resend_verification_code_post(){
	    ...
	    $query = $this->db->table('users')->where($credentials)->get();
	    if($query->getNumRows() > 0) {
	    	$this->email_model->send_email_verification_mail($check['email'], $query->getRow()->verification_code);
	...
	{
		$my_courses = array();
		$my_courses_ids = $this->user_model->my_courses($user_id)->getResultArray();
		foreach ($my_courses_ids as $my_courses_id) {
			$course_details = $this->crud_model->get_course_by_id($my_courses_id['course_id'])->getRowArray();
		...
		foreach ($my_courses as $key => $my_course) {
			if (isset($my_course['id']) && $my_course['id'] > 0) {
				...
				$my_courses[$key]['total_number_of_lessons'] = $this->crud_model->get_lessons('course', $my_course['id'])->getNumRows();
	...
	public function sections_get($course_id = "", $user_id = "")
	{
		...
		$sections = $this->crud_model->get_section('course', $course_id)->getResultArray();
	...
	{
		$response = array();
		$lessons = $this->crud_model->get_lessons('section', $section_id)->getResultArray();
	...
	public function get_completed_number_of_lesson($user_id = "", $type = "", $id = "")
	{
		$counter = 0;
		if ($type == 'section') {
			$lessons = $this->crud_model->get_lessons('section', $id)->getResultArray();
		} else {
			$lessons = $this->crud_model->get_lessons('course', $id)->getResultArray();
		}
		foreach ($lessons as $key => $lesson) {
	...
	{
		$lesson_details = $this->crud_model->get_lessons('lesson', $lession_id)->getResultArray();
	...
	{
		$course_details = $this->crud_model->get_course_by_id($course_id)->getResultArray();
		...
		foreach ($response as $key => $resp) {
			...
			$response[$key]['includes'] = array(
				...
				$this->crud_model->get_lessons('course', $course_id)->getNumRows() . ' Lessons',
				'High quality videos',
				'Life time access'
			);
		}
	...
	public function is_added_to_wishlist($user_id = 0, $course_id = "")
	{
		if ($user_id > 0) {
			$wishlists = array();
			$user_details = $this->user_model->get_all_user($user_id)->getRowArray();
			$wishlists = json_decode($user_details['wishlist']);
	...
	public function course_object_by_id_get()
	{
		...
		$course['total_enrollment'] = $this->crud_model->enrol_history($course['id'])->getNumRows();
	...
	private function course_completion_data($course_id = "", $user_id = "")
	{
		...
		$response['number_of_lessons'] = $this->crud_model->get_lessons('course', $course_id)->getNumRows();
	...
	public function certificate_addon_get($user_id = "", $course_id = "")
	{
		...
		if (addon_status('certificate')) {
			...
			if ($course_progress == 100) {
				...
				$previous_data = $this->db->table('certificates')->where($checker)->get();
				if ($previous_data->getNumRows() == 0) {
					...
					$previous_data = $previous_data->getRowArray();
...
//Start bundle addon
	...
	public function bundles_get($limit ="")
	{
		...
		$bundle_courses = $this->db->table('course_bundle')->get()->getResultArray();
		...
		foreach ($bundle_courses as $key => $bundle_course) {
			...
			if ($ratings->getNumRows() > 0) {
				$bundle_course['average_rating'] = ceil($bundle_total_rating / $ratings->getNumRows());
				$bundle_course['number_of_ratings'] = $ratings->getNumRows();
	...
	public function bundle_courses_get($bundle_id = "", $user_id = "")
	{
		...
		$this->db->where('status', 1);
		$bundle_details 		= $this->db->table('course_bundle')->get()->getRowArray();
		$user_details 		= $this->db->table('users')->where(array('id' => $bundle_details['user_id']))->get()->getRowArray();
		...
		if ($ratings->getNumRows() > 0) {
			$result['average_rating'] = ceil($bundle_total_rating / $ratings->getNumRows());
			$result['number_of_ratings'] = $ratings->getNumRows();
		}else {
		...
		$this->db->whereIn('id', $bundle_course_ids);
		$this->db->where('status', 'active');
		$courses = $this->db->table('course')->get()->getResultArray();
	...
	public function my_bundles_get($user_id = ""){
		...
		$bundle_payments = $this->db->table('bundle_payment')->get()->getResultArray();
		...
		foreach ($bundle_payments as $key => $bundle_payment) {
			...
			$bundle_total_rating = $this->course_bundle_model->sum_of_bundle_rating($bundle_details['id']);
			if ($ratings->getNumRows() > 0) {
				$bundle_details['average_rating'] = ceil($bundle_total_rating / $ratings->getNumRows());
				$bundle_details['number_of_ratings'] = $ratings->getNumRows();
			}else {
	...
	public function my_bundle_course_details_get($user_id= "", $bundle_id = "", $course_id = "")
	{
		...
		foreach ($my_bundle_course_details as $key => $my_course) {
			if (isset($my_course['id']) && $my_course['id'] > 0) {
				...
				$my_bundle_course_details[$key]['total_number_of_lessons'] = $this->crud_model->get_lessons('course', $my_course['id'])->getNumRows();
...
//End Bundle
  ...
  //Start Form addon
	public function forum_add_questions_post($user_id = "", $course_id = "") {
	    ...
	    $this->db->table('course_forum')->insert($data);
	...
	public function forum_questions_get($user_id, $course_id = "", $page_number = 0, $limit = 20){
		...
		$questions = $this->db->table('course_forum')->get();

		foreach($questions->getResultArray() as $key => $question):
			$user_details 		= $this->db->table('users')->where(array('id' => $question['user_id']))->get()->getRowArray();
			...
			$question_arr[$key]['comment_number'] = $this->db->table('course_forum')->where(array('is_parent' => $question['id']))->get()->getNumRows();
			if(is_array($upvoted_user_arr)){
	...
	public function search_forum_questions_get($user_id, $course_id = ""){
		...
		$this->db->orLike('description', $search_val);
	...
	public function add_questions_reply_post($user_id = "", $parent_id = "") {
	    ...
	    $data['user_id'] = $user_id;
	    $data['course_id'] = $this->db->table('course_forum')->where(array('id' => $parent_id))->get()->getRow()->course_id;
	    $data['description'] = $this->request->getPost('description');
	...
	public function forum_child_questions_get($parent_question_id = "") {
		...
		$child_questions = $this->db->table('course_forum')->get();

		foreach($child_questions->getResultArray() as $key => $question):
			$user_details 		= $this->db->table('users')->where(array('id' => $question['user_id']))->get()->getRowArray();
			$question_arr[$key] = $question;
			$question_arr[$key]['user_name'] = $user_details['first_name'].' '.$user_details['last_name'];
			$question_arr[$key]['user_image'] = $this->user_model->get_user_image_url($question['user_id']);
	...
	public function forum_question_vote_get($user_id = "", $question_id = ""){
        ...
        $upvoted_user_ids = $this->db->get_where('course_forum')->getRow()->upvoted_user_id;
        ...
        $this->db->where('id', $question_id);
        $this->db->table('course_forum')->update($data);
	...
	public function forum_question_delete_get($user_id = "", $question_id = ""){
		...
		$this->db->where('user_id', $user_id);
		$this->db->where('id', $question_id);
        $this->db->table('course_forum')->delete();
        return $response;
	}
	...
	function update_watch_history_with_duration_post($user_id = "")
    {
        ...
        if ($current_history->getNumRows() > 0) {
            ...
            $this->db->table('watched_duration')->update(array('watched_counter' => $watched_duration_json));
        } else {
        ...
        if ($is_completed == 1) {
            ...
            if ($query->getNumRows() > 0) {
                ...
                if (!in_array($data['watched_lesson_id'], $lesson_ids)) {
                    ...
                    $this->db->table('watch_histories')->update(array('course_progress' => $course_progress, 'completed_lesson' => json_encode($lesson_ids), 'date_updated' => time()));
...
//Logged in from mobile app only for web view. Avoite tracking multiple device login for mbile app web view
    function login_for_web_view($user_id = ""){
    	...
    	if($query->getNumRows() > 0){
	    	...
	    	if($this->session->get('user_login') != '1'){
				...
				$this->session->set('custom_session_limit', (time()+43200));
	...
	function get_all_blogs(){
		$all_blogs_data = array();
		$blogs = $this->crud_model->get_all_blogs();
		
		foreach($blogs->getResultArray() as $blog):
			//print_r($blog);exit;
			$blog_data = array();
			$user_details = $this->user_model->get_all_user($blog['user_id'])->getRowArray();
			$blog_data['site_details'] = site_url('blog/details/'.slugify($blog['title']).'/'.$blog['blog_id']);
			$blog_thumbnail = 'uploads/blog/thumbnail/'.$blog['thumbnail'];
			
			if(file_exists($blog_thumbnail) && is_file($blog_thumbnail)):
				$blog_data['blog_thumbnail_url'] = base_url($blog_thumbnail);
			else:
				$blog_data['blog_thumbnail_url'] = base_url('uploads/blog/thumbnail/placeholder.png');
			endif;
			
			$blog_data['blog_id'] = $blog['blog_id'];
			$blog_data['blog_title'] = $blog['title'];
			
			$blog_data['meta_description'] = $blog['meta_description'];
			$blog_data['meta_keyword'] = $blog['meta_keyword'];
			$blog_data['meta_title'] = $blog['meta_title'];
				
			$blog_data['blog_category_title'] = $this->crud_model->get_blog_categories($blog['blog_category_id'])->getRow()->title;

			$blog_data['blog_description'] = ellipsis(strip_tags(htmlspecialchars_decode_($blog['description'])), 150);
			$blog_data['blog_writer_image'] = $this->user_model->get_user_image_url($user_details['id']);
			$blog_data['blog_writer'] = $user_details['first_name'].' '.$user_details['last_name'];
			$blog_data['blog_date'] = get_past_time($blog['added_date']); 
				
			$all_blogs_data[] = $blog_data;
		endforeach;
		
//print_r($all_blogs_data);exit;
