...
class Social_login_modal extends NativeBaseModel
{
    ...
    public function fb_validate_login($access_token = "", $fb_user_id = "") {
        ...
        if(isset($user['email'])){
            if(filter_var($user['email'], FILTER_VALIDATE_EMAIL)){
                ...
                $this->session->setFlashdata('error_message', get_phrase('invalid_email_address'));
                redirect(site_url('login'), 'refresh');
            }
        }else{
            $this->delete_app($fb_user_id, $access_token);
            $this->session->setFlashdata('error_message', get_phrase('email_access_permission_is_required'));
            redirect(site_url('login'), 'refresh');
        }
