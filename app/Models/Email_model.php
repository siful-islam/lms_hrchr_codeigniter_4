...
class Email_model extends NativeBaseModel
{
	...
	public function send_email_verification_mail($to = "", $verification_code = "")
	{
		...
		$notification = $this->db->where('type', $type)->get('notification_settings')->getRowArray();
		foreach(json_decode($notification['user_types'], true) as $user_type){
			
			//Editable
			if($user_type == 'user'){
				$to_user = $this->db->table('users')->where(array('email' => $to))->get()->getRowArray();
	...
	function signup_mail($new_user_id = ""){
		...
		foreach(json_decode($notification['user_types'], true) as $user_type){
			...
			//Editable
			if($user_type == 'admin'){
				$new_user = $this->db->table('users')->where(array('id' => $new_user_id))->get()->getRowArray();
				$to_user = $this->db->table('users')->where(array('role_id' => 1))->get()->getRowArray();
				$replaces['user_name'] = $new_user['first_name'].' '.$new_user['last_name'];
				$replaces['user_email'] = $new_user['email'];
			}
			if($user_type == 'user'){
				$to_user = $this->db->table('users')->where(array('id' => $new_user_id))->get()->getRowArray();
	...
	function password_reset_email($verification_code = '', $to = '')
	{
		...
		foreach(json_decode($notification['user_types'], true) as $user_type){
			...
			//Editable
			if($user_type == 'user'){
				$query = $this->db->table('users')->where(array('email' => $to))->get();
				$to_user = $query->getRowArray();
	...
	public function send_mail_on_course_status_changing($course_id = "", $mail_subject = "", $mail_body = "")
	{
		...
		$instuctor_details = $this->user_model->get_all_user($instructor_id)->getRowArray();
		...
		$admin = $this->db->table('users')->where(array('role_id' => 1))->get()->getRowArray();
	...
	public function course_purchase_notification($student_id = "", $payment_method = "", $amount_paid = "")
	{
		...
		// $student_data 		= $this->user_model->get_all_user($student_id)->getRowArray();
		// $student_full_name 	= $student_data['first_name'] . ' ' . $student_data['last_name'];
		// $admin_id 			= $this->user_model->get_admin_details()->getRow()->id;
		// foreach ($purchased_courses as $course_id) {
		// 	$course_owner_user_id = $this->crud_model->get_course_by_id($course_id)->getRow()->user_id;
		...
		$notification = $this->db->where('type', $type)->get('notification_settings')->getRowArray();
		foreach($this->session->get('cart_items') as $course_id){
			foreach(json_decode($notification['user_types'], true) as $user_type){
				
				//Editable
				if($user_type == 'admin'){
					$course_details = $this->crud_model->get_course_by_id($course_id)->getRowArray();
					$student_details = $this->user_model->get_all_user($student_id)->getRowArray();
					$instructor_details = $this->user_model->get_all_user($course_details['creator'])->getRowArray();
					$to_user = $this->db->table('users')->where(array('role_id' => 1))->get()->getRowArray();
				...
				if($user_type == 'instructor'){
					$course_details = $this->crud_model->get_course_by_id($course_id)->getRowArray();
					$student_details = $this->user_model->get_all_user($student_id)->getRowArray();
					$to_user = $this->user_model->get_all_user($course_details['creator'])->getRowArray();
					$replaces['course_title'] = '<a href="'.site_url('home/course/'.slugify($course_details['title']).'/'.$course_details['id']).'" target="_blank">'.$course_details['title'].'</a>';
					$replaces['student_name'] = $student_details['first_name'].' '.$student_details['last_name'];
					$replaces['paid_amount'] = $amount_paid;
				}
				if($user_type == 'student'){
					$course_details = $this->crud_model->get_course_by_id($course_id)->getRowArray();
					$instructor_details = $this->user_model->get_all_user($course_details['creator'])->getRowArray();
					$to_user = $this->user_model->get_all_user($student_id)->getRowArray();
	...
	public function notify_on_certificate_generate($student_id = "", $course_id = "")
	{
		...
		foreach(json_decode($notification['user_types'], true) as $user_type){
			...
			if($user_type == 'instructor'){
				$certificate = $this->db->table('certificates')->where(array('course_id' => $course_id,'student_id' => $student_id))->get()->getRowArray();
				$course_details = $this->crud_model->get_course_by_id($course_id)->getRowArray();
				$student_details = $this->user_model->get_all_user($student_id)->getRowArray();
				$to_user = $this->user_model->get_all_user($course_details['creator'])->getRowArray();
				$replaces['course_title'] = '<a href="'.site_url('home/course/'.slugify($course_details['title']).'/'.$course_details['id']).'" target="_blank">'.$course_details['title'].'</a>';
			...
			if($user_type == 'student'){
				$certificate = $this->db->table('certificates')->where(array('course_id' => $course_id,'student_id' => $student_id))->get()->getRowArray();
				$course_details = $this->crud_model->get_course_by_id($course_id)->getRowArray();
				$instructor_details = $this->user_model->get_all_user($student_id)->getRowArray();
				$to_user = $this->user_model->get_all_user($student_id)->getRowArray();
				$replaces['course_title'] = '<a href="'.site_url('home/course/'.slugify($course_details['title']).'/'.$course_details['id']).'" target="_blank">'.$course_details['title'].'</a>';
				$replaces['instructor_name'] = $instructor_details['first_name'].' '.$instructor_details['last_name'];
	...
	public function bundle_purchase_notification($student_id = "", $payment_method = "", $amount_paid = "")
	{
		...
		$bundle_id = $this->session->get('checkout_bundle_id');
		$bundle_details = $this->course_bundle_model->get_bundle($bundle_id)->getRowArray();
		$admin_details = $this->user_model->get_admin_details()->getRowArray();
		$bundle_creator_details = $this->user_model->get_all_user($bundle_details['user_id'])->getRowArray();
		$student_details = $this->user_model->get_all_user($student_id)->getRowArray();
	...
	function send_notice($notice_id = "", $course_id = "")
	{
		...
		foreach(json_decode($notification['user_types'], true) as $user_type){
			...
			if($user_type == 'student'){
				$course_details = $this->crud_model->get_course_by_id($course_id)->getRowArray();
				$notice_details = $this->noticeboard_model->get_notices($notice_id)->getRowArray();
				$instructor_details = $this->user_model->get_all_user($course_details['user_id'])->getRowArray();
				$enrolled_students = $this->crud_model->enrol_history($course_id)->getResultArray();

				//End editable

				foreach ($enrolled_students as $enrolled_student) :
					$to_user = $this->user_model->get_user($enrolled_student['user_id'])->getRowArray();
					$replaces['instructor_name'] = $instructor_details['first_name'] . ' ' . $instructor_details['last_name'];
	...
	{
		$student_details = $this->db->table('users')->where(array('email' => $to))->get()->getRowArray();
		$email_data['subject'] = 'Your live class started';
		$email_data['message'] = $this->request->getPost('jitsi_live_alert_message');
		$email_template = $this->load->view('email/static_common_template', $email_data, TRUE);
	...
	function new_device_login_alert($user_id = "")
	{
		...
		if (!$this->session->get('new_device_verification_code')) {
			$new_device_verification_code = rand(100000, 999999);
		} else {
			$new_device_verification_code = $this->session->get('new_device_verification_code');
		}
		if ($user_id == "") {
			$user_id = $this->session->get('new_device_user_id');
		}
		$row = $this->db->table('users')->where(array('id' => $user_id))->get()->getRowArray();
		...
		$this->session->set('new_device_code_expiration_time', (time() + 600));
		$this->session->set('new_device_user_email', $row['email']);
		$this->session->set('new_device_user_id', $user_id);
		$this->session->set('new_device_verification_code', $new_device_verification_code);
	...
	public function become_a_course_affiliator_by_admin($email = "", $name = "", $password = "")
	{
		...
		//End editable
		$affiliator = $this->db->table('users')->where(array('email' => $email))->get()->getRowArray();
		$to_user = $affiliator;
	...
	public function send_email_when_make_withdrawl_request($email = "", $name = "",$amount="")
	{
		...
		foreach(json_decode($notification['user_types'], true) as $user_type){
			if($user_type == 'admin'){
				...
				$to_user = $this->user_model->get_admin_details()->getRowArray();
			}
			if($user_type == 'affiliator'){
				$replaces['amount'] = currency($amount);
				$to_user = $this->db->table('users')->where(array('email' => $email))->get()->getRowArray();
			}
			$template_data['replaces'] = isset($replaces) ? $replaces:array();
			$template_data['to_user'] = $affiliator;
			$template_data['notification'] = $notification;
	...
	public function course_gift_notification($enrol_student_id="", $payer_user_id = "")
	{
		...
		foreach($this->session->get('cart_items') as $course_id){
			$course_details = $this->crud_model->get_course_by_id($course_id)->getRowArray();
			$payer_details = $this->user_model->get_all_user($payer_user_id)->getRowArray();
			$enrol_student_details = $this->user_model->get_all_user($enrol_student_id)->getRowArray();
			$instructor_details = $this->user_model->get_all_user($course_details['user_id'])->getRowArray();
			...
			foreach(json_decode($notification['user_types'], true) as $user_type){
				if($user_type == 'payer'){
					...
					$replaces['course_title'] = '<a href="'.site_url('home/course/'.slugify($course_details['title'])).'/'.$course_details['id'].'" target="_blank">'.$course_details['title'].'</a>';
					$to_user = $this->user_model->get_all_user($payer_user_id)->getRowArray();
				}
				if($user_type == 'receiver'){
					$replaces['payer'] = $instructor_details['first_name'].' '.$payer_details['last_name'];
					$replaces['instructor'] = $instructor_details['first_name'].' '.$instructor_details['last_name'];
					$replaces['course_title'] = '<a href="'.site_url('home/course/'.slugify($course_details['title'])).'/'.$course_details['id'].'" target="_blank">'.$course_details['title'].'</a>';
					$to_user = $this->user_model->get_all_user($enrol_student_id)->getRowArray();
				}
				$template_data['replaces'] = isset($replaces) ? $replaces:array();
				$template_data['to_user'] = $to_user;
				$template_data['notification'] = $notification;
	...
	function course_completion($user_id = "", $course_id = ""){
		...
		$course_details = $this->db->table('course')->where(['id' => $course_id])->get()->getRowArray();
		$instructor = $this->user_model->get_all_user($course_details['creator'])->getRowArray();
		...
		foreach(json_decode($notification['user_types'], true) as $user_type){
			if($user_type == 'student'){
				...
				$to_user = $this->user_model->get_all_user($user_id)->getRowArray();
			}
			if($user_type == 'instructor'){
				$replaces['course_title'] = '<a href="'.site_url('home/course/'.slugify($course_details['title'])).'/'.$course_details['id'].'" target="_blank">'.$course_details['title'].'</a>';
				$replaces['student_name'] = $user_details['first_name'].' '.$user_details['last_name'];
				$to_user = $this->user_model->get_all_user($instructor['id'])->getRowArray();
			}
			$template_data['replaces'] = isset($replaces) ? $replaces:array();
			$template_data['to_user'] = $affiliator;
			$template_data['notification'] = $notification;
	...
	function instructor_followups($instructor_id = "", $course_id = ""){
		...
		$following_users = $this->db->table('instructor_followings')->where(['instructor_id' => $instructor_id, 'is_following' => 1])->get()->getResultArray();
	
		foreach(json_decode($notification['user_types'], true) as $user_type) {
			if($user_type == 'student'){
				foreach($following_users as $follower) {
					$instructor = $this->user_model->get_all_user($follower['instructor_id'])->getRowArray();
					$replaces['course_title'] = '<a href="'.site_url('home/course/'.slugify($course_details['title'])).'/'.$course_details['id'].'" target="_blank">'.$course_details['title'].'</a>';
					//$replaces['student_name'] = $student_details['first_name'].' '.$student_details['last_name'];
					$replaces['instructor_name'] = $instructor['first_name'].' '.$instructor['last_name'];
					$to_user = $this->user_model->get_all_user($instructor_id)->getRowArray();
	...
	public function instructor_followups_reminder($instructor_id, $user_id) {
		...
		foreach (json_decode($notification['user_types'], true) as $user_type) {
			if ($user_type == 'instructor') {
				foreach ($following_users as $follower) {
					$student_details = $this->user_model->get_all_user($follower['user_id'])->getRowArray();
					$replaces['course_title'] = '<a href="'.site_url('home/course/'.slugify($course_details['title'])).'/'.$course_details['id'].'" target="_blank">'.$course_details['title'].'</a>';
					$replaces['student_name'] = $student_details['first_name'].' '.$student_details['last_name'];
					$to_user = $this->user_model->get_all_user($user_id)->getRowArray();
	...
	function notify($type = "", $user_id = "", $subject = "", $description = "", $from_user = ""){
        if($from_user == "" && $this->session->get('user_id') > 0){
            $from_user = $this->session->get('user_id');
        }else{
        	$from_user = $this->db->table('users')->where(['role_id' => 1])->get()->getRow()->id;
        }
