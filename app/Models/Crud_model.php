...
//v5.7
class Crud_model extends NativeBaseModel
{
...
function __construct()
    ...
    {
        $builder = $this->qb('category');

        if ($param1 !== "") {
            $builder->where('id', $param1);
        }

        return $builder->where('parent', 0)->get();
    }

    public function get_category_details_by_id($id)
    {
        return $this->db->table('category')->where(array('id' => $id))->get();
    }

    public function get_category_id($slug = "")
    {
        $category_details = $this->db->table('category')->where(array('slug' => $slug))->get()->getRowArray();
    ...
    public function add_category()
    {
        ...
        $data['parent'] = html_escape($this->request->getPost('parent'));
        $data['slug']   = slugify(html_escape($this->request->getPost('name')));

        // CHECK IF THE CATEGORY NAME ALREADY EXISTS
        $this->db->where('name', $data['name']);
        $this->db->orWhere('slug', $data['slug']);
        $previous_data = $this->db->table('category')->get()->getNumRows();

        if ($previous_data == 0) {
            // Font awesome class adding
            if ($_POST['font_awesome_class'] != "") {
                $data['font_awesome_class'] = html_escape($this->request->getPost('font_awesome_class'));
            } else {
                $data['font_awesome_class'] = 'fas fa-chess';
            }

            if ($this->request->getPost('parent') == 0) {
    ...
    public function edit_category($param1)
    {
        ...
        $this->db->where('name', $data['name']);
        $this->db->orWhere('slug', $data['slug']);
        $previous_data = $this->db->table('category')->get()->getResultArray();
        ...
        if ($checker) {
            ...
            $this->db->table('category')->update($data);
   ...
   // Category Sub Category Image Delete
    ...
    {
        $this->db->where('id', $category_id);
        $this->db->table('category')->update(array('sub_category_thumbnail' => NULL));
    }
    ...
    {
        return $this->db->table('category')->where(array('parent' => $parent_id))->get()->getResultArray();
    }

   public function enrol_history($course_id = "", $distinct_data = false)
	{
		$builder = $this->db->table('enrol');

		if ($distinct_data) {
			$builder->select('user_id')->distinct();
    ...
    public function all_enrolled_student()
    {
        ...
        return $this->db->table('enrol')->get();
    }
    ...
    public function get_revenue_by_user_type($timestamp_start = "", $timestamp_end = "", $revenue_type = "")
    {
        ...
        if ($revenue_type == 'admin_revenue') {
            ...
            if (sizeof($course_ids)) {
                $this->db->whereIn('course_id', $course_ids);
            } else {
                return array();
            }
        }
        ...
        $this->db->orderBy('date_added', 'desc');
        return $this->db->table('payment')->get()->getResultArray();
    }
    ...
    public function get_instructor_revenue($user_id = "", $timestamp_start = "", $timestamp_end = "")
    {
        ...
        if ($user_id > 0) {
            ...
            $this->db->where('creator', $this->session->get('user_id'));
        }
    ...
    public function delete_enrol_history($param1)
    {
        ...
        $this->db->where('id', $param1);
        $result = $this->db->table('enrol')->get()->getRowArray();
        ...
        if ($result) {
            ...
            if (!empty($lesson_ids)) {
                ...
                $this->db->whereIn('quiz_id', $quiz_ids);
                $this->db->where('user_id', $result['user_id']);
                $this->db->table('quiz_results')->delete();
            }

            // Delete the enrol record
            $this->db->where('id', $param1);
            $this->db->table('enrol')->delete();
        }
    }
    ...
    {
        if ($user_id > 0) {
            return $this->db->table('payment')->where(array('user_id' => $user_id))->get();
        } else {
            return $this->db->table('payment')->get();
        }
    }

    public function get_payment_details_by_id($payment_id = "")
    {
        return $this->db->table('payment')->where(array('id' => $payment_id))->get()->getRowArray();
    }
    ...
    public function update_payout_status($payout_id = "", $payment_type = "")
    {
        ...
        $this->db->table('payout')->update($updater);
    }

    public function update_system_settings()
    {
        $data['value'] = html_escape($this->request->getPost('system_name'));
        $this->db->where('key', 'system_name');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('system_title'));
        $this->db->where('key', 'system_title');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('author'));
        $this->db->where('key', 'author');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('slogan'));
        $this->db->where('key', 'slogan');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('language'));
        $this->db->where('key', 'language');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('text_align'));
        $this->db->where('key', 'text_align');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('system_email'));
        $this->db->where('key', 'system_email');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('address'));
        $this->db->where('key', 'address');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('phone'));
        $this->db->where('key', 'phone');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('youtube_api_key'));
        $this->db->where('key', 'youtube_api_key');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('vimeo_api_key'));
        $this->db->where('key', 'vimeo_api_key');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('purchase_code'));
        $this->db->where('key', 'purchase_code');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('footer_text'));
        $this->db->where('key', 'footer_text');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('footer_link'));
        $this->db->where('key', 'footer_link');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('website_keywords'));
        $this->db->where('key', 'website_keywords');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('website_description'));
        $this->db->where('key', 'website_description');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('student_email_verification'));
        $this->db->where('key', 'student_email_verification');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('course_accessibility'));
        $this->db->where('key', 'course_accessibility');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('allowed_device_number_of_loging'));
        ...
        $data['value'] = html_escape($this->request->getPost('course_selling_tax'));
        $this->db->where('key', 'course_selling_tax');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('google_analytics_id'));
        $this->db->where('key', 'google_analytics_id');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('meta_pixel_id'));
        $this->db->where('key', 'meta_pixel_id');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('public_signup'));
        $this->db->where('key', 'public_signup');
        $this->db->table('settings')->update($data);

        $this->db->where('key', 'account_disable');
        $row = $this->db->table('settings')->get();
        if ($row->getNumRows() > 0) {
            $this->db->where('key', 'account_disable');
            $this->db->table('settings')->update(['value' => $this->request->getPost('account_disable')]);
        } else {
            $this->db->table('settings')->insert(['key' => 'account_disable', 'value' => $this->request->getPost('account_disable')]);
        }
        ...
        if ($row->getNumRows() > 0) {
            $this->db->where('key', 'timezone');
            $this->db->table('settings')->update(['value' => $this->request->getPost('timezone')]);
        } else {
            $this->db->table('settings')->insert(['key' => 'timezone', 'value' => $this->request->getPost('timezone')]);
        }
    }

    public function update_smtp_settings()
    {
        $data['value'] = html_escape($this->request->getPost('protocol'));
        $this->db->where('key', 'protocol');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('smtp_crypto'));
        $this->db->where('key', 'smtp_crypto');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('smtp_host'));
        $this->db->where('key', 'smtp_host');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('smtp_port'));
        $this->db->where('key', 'smtp_port');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('smtp_from_email'));
        $this->db->where('key', 'smtp_from_email');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('smtp_user'));
        $this->db->where('key', 'smtp_user');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('smtp_pass'));
    ...
    {
        $data['value'] = html_escape($this->request->getPost('fb_social_login'));
        $this->db->where('key', 'fb_social_login');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('fb_app_id'));
        $this->db->where('key', 'fb_app_id');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('fb_app_secret'));
    ...
    function get_payment_gateways($identifier = "")
    {
        ...
        return $this->db->table('payment_gateways')->get();
    }

    public function update_payment_settings()
    {
        $keys = array();
        $data['title'] = ucfirst($this->request->getPost('identifier'));
        $data['identifier'] = $this->request->getPost('identifier');
        $data['currency'] = $this->request->getPost('currency');
        $data['enabled_test_mode'] = $this->request->getPost('enabled_test_mode');
        ...
        if ($this->get_payment_gateways($data['identifier'])->getNumRows() > 0) {
            $data['updated_at'] = time();
            $this->db->where('identifier', $data['identifier']);
            $this->db->table('payment_gateways')->update($data);
        } else {
            $data['created_at'] = time();
            $this->db->table('payment_gateways')->insert($data);
        }
        $this->session->setFlashdata('flash_message', get_phrase('payment_settings_updated_successfully'));
    }
    ...
    public function update_stripe_settings()
    {
        ...
        $stripe['active'] = $this->request->getPost('stripe_active');
        $stripe['testmode'] = $this->request->getPost('testmode');
        $stripe['public_key'] = $this->request->getPost('public_key');
        $stripe['secret_key'] = $this->request->getPost('secret_key');
        $stripe['public_live_key'] = $this->request->getPost('public_live_key');
        $stripe['secret_live_key'] = $this->request->getPost('secret_live_key');
        ...
        $data['value'] = html_escape($this->request->getPost('stripe_currency'));
    ...
    public function update_razorpay_settings()
    {
        ...
        $paytm_info = array();
        $razorpay['active'] = htmlspecialchars_($this->request->getPost('razorpay_active'));
        $razorpay['key'] = htmlspecialchars_($this->request->getPost('key'));
        $razorpay['secret_key'] = htmlspecialchars_($this->request->getPost('secret_key'));
        $razorpay['theme_color'] = htmlspecialchars_($this->request->getPost('theme_color'));
        ...
        $data['value'] = htmlspecialchars_($this->request->getPost('razorpay_currency'));
    ...
    {
        $data['value'] = html_escape($this->request->getPost('system_currency'));
        $this->db->where('key', 'system_currency');
        $this->db->table('settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('currency_position'));
    ...
    public function update_instructor_settings()
    {
        if (isset($_POST['allow_instructor'])) {
            $data['value'] = html_escape($this->request->getPost('allow_instructor'));
            $this->db->where('key', 'allow_instructor');
            $this->db->table('settings')->update($data);
        }

        if (isset($_POST['instructor_revenue'])) {
            $data['value'] = html_escape($this->request->getPost('instructor_revenue'));
            $this->db->where('key', 'instructor_revenue');
            $this->db->table('settings')->update($data);
        }

        if (isset($_POST['instructor_application_note'])) {
            $data['value'] = html_escape($this->request->getPost('instructor_application_note'));
    ...
    {
        $builder = $this->qb('lesson')->orderBy('order', 'asc');
    ...
    {
        $faqs = array();
        if (!empty($this->request->getPost('faqs'))) :
            foreach (array_filter($this->request->getPost('faqs')) as $faq_key => $faq_title) {
                $faqs[$faq_title] = $this->request->getPost('faq_descriptions')[$faq_key];
            }
        endif;
        ...
        $outcomes = $this->trim_and_return_json($this->request->getPost('outcomes'));
        $requirements = $this->trim_and_return_json($this->request->getPost('requirements'));

        $data['course_type'] = html_escape($this->request->getPost('course_type'));
        ...
        $data['description'] = remove_js($this->request->getPost('description', false));
        $data['outcomes'] = $outcomes;
        $data['faqs'] = json_encode($faqs);
        $data['language'] = $this->request->getPost('language_made_in');
        $data['sub_category_id'] = $this->request->getPost('sub_category_id');
        $category_details = $this->get_category_details_by_id($this->request->getPost('sub_category_id'))->getRowArray();
        ...
        $data['video_url'] = html_escape($this->request->getPost('course_overview_url'));

        $enable_drip_content = $this->request->getPost('enable_drip_content');
        ...
        $data['creator'] = $this->session->get('user_id');
        ...
        if ($data['status'] == 'approved') {
            $this->session->setFlashdata('flash_message', get_phrase('course_added_successfully'));
        } elseif ($data['status'] == 'pending') {
            $this->session->setFlashdata('flash_message', get_phrase('course_added_successfully') . '. ' . get_phrase('please_wait_untill_Admin_approves_it'));
        } elseif ($data['status'] == 'draft') {
            $this->session->setFlashdata('flash_message', get_phrase('your_course_has_been_added_to_draft'));
        }

        $this->session->setFlashdata('flash_message', get_phrase('course_has_been_added_successfully'));
        return $course_id;
    }
    ...
    function add_shortcut_course($param1 = "")
    {
        ...
        if ($this->request->getPost('is_private') == 1) {
            $data['status'] = 'private';
        } else {
    ...
    {
        $course_details = $this->get_course_by_id($course_id)->getRowArray();
    ...
    if ($_FILES['upcoming_image_thumbnail']['name'] == "") {
        ...
        if ($data['status'] == 'active') {
            $this->session->setFlashdata('flash_message', get_phrase('course_updated_successfully'));
        } elseif ($data['status'] == 'pending') {
            $this->session->setFlashdata('flash_message', get_phrase('course_updated_successfully') . '. ' . get_phrase('please_wait_untill_Admin_approves_it'));
        } elseif ($data['status'] == 'draft') {
            $this->session->setFlashdata('flash_message', get_phrase('your_course_has_been_added_to_draft'));
        }
    }
    ...
    function get_course_thumbnail_url($course_id, $type = 'course_thumbnail')
    {
        ...
        $last_modified = $this->get_course_by_id($course_id)->getRow()->last_modified;
    ...
    public function get_my_courses_by_category_id($category_id)
    {
        ...
        $course_lists_by_enrol = $this->db->table('enrol')->where(array('user_id' => $this->session->get('user_id'))->get())->getResultArray();
        $course_ids = array();
        ...
        return $this->db->table('course')->get();
    }
    ...
    public function get_courses_by_search_string($search_string = "", $per_page = "", $uri_segment = "")
    {
        ...
        $this->db->groupStart();
        $this->db->like('title', $search_string);
        $this->db->orLike('short_description', $search_string);
        $this->db->orLike('description', $search_string);
        $this->db->orLike('outcomes', $search_string);
        $this->db->orLike('language', $search_string);
        $this->db->orLike('requirements', $search_string);
        $this->db->orLike('meta_keywords', $search_string);
        $this->db->orLike('meta_description', $search_string);
        $this->db->groupEnd();
    ...
    {
        $builder = $this->qb('course');

        if (is_array($course_id)) {
            return $builder->whereIn('id', $course_id)->get();
        }

        return $builder->where('id', $course_id)->get();
    }

    public function delete_course($course_id = "")
    {
        $course_type = $this->get_course_by_id($course_id)->getRow()->course_type;

        $this->db->where('id', $course_id);
        $this->db->table('course')->delete();
        ...
        if ($course_type == 'general') {
            ...
            $this->db->table('lesson')->delete($lesson_checker);
            ...
            $this->db->where('course_id', $course_id);
            $this->db->table('scorm_curriculum')->delete();

            if ($scorm_query->getNumRows() > 0) {
                //deleted previews course directory
                $this->scorm_model->deleteDir('uploads/scorm/courses/' . $scorm_query->getRow()->identifier);
            }
	...
	public function get_top_courses()
	{
		...
		$builder->groupStart()
			->where('course_type', 'general');

		if ($scorm_status) {
			$builder->orWhere('course_type', 'scorm');
		}

		if ($h5p_status) {
			$builder->orWhere('course_type', 'h5p');
		}
    ...
    {
        $categories = $this->get_categories()->getResultArray();
    ...
    public function get_status_wise_courses_front($status = "")
    {
        ...
        if ($status != "") {
            ...
            $this->db->groupEnd();
            $courses = $this->db->table('course')->get();
        } else {
            //draft
            ...
            $this->db->groupEnd();
            $courses['draft'] = $this->db->table('course')->get();
            ...
            $this->db->groupEnd();
            $courses['pending'] = $this->db->table('course')->get();
            ...
            $this->db->groupEnd();
            $courses['private'] = $this->db->table('course')->get();
            ...
            $this->db->groupEnd();
            $courses['active'] = $this->db->table('course')->get();
            ...
            $this->db->groupEnd();
            $courses['upcoming'] = $this->db->table('course')->get();
        }
    ...
    public function get_status_wise_courses_for_instructor($status = "")
    {
        ...
        if ($status != "") {
            $courses = $this->get_courses_by_instructor_id($this->session->get('user_id'), $status);
        } else {
    ...
    public function get_instructor_wise_payment_history($instructor_id = "")
    {
        ...
        if (sizeof($courses) > 0) {
            $this->db->whereIn('course_id', $courses);
            return $this->db->table('payment')->get()->getResultArray();
        } else {
            return array();
        }
    ...
    {
        $date_range_with_time = $this->request->getPost('date_range_of_study_plan');
        if ($date_range_with_time != '') {
            ...
            $data['restricted_by'] = $this->request->getPost('restricted_by');
        }
    ...
    public function get_section($type_by, $id)
    {
        ...
        if ($type_by == 'course') {
            return $this->db->table('section')->where(array('course_id' => $id))->get();
        } elseif ($type_by == 'section') {
            return $this->db->table('section')->where(array('id' => $id))->get();
        }
    }
    ...
    public function add_lesson()
    {
        ...
        if ($lesson_type == 'video') {
            ...
            if ($lesson_provider == 'youtube' || $lesson_provider == 'vimeo') {
                ...
                if ($this->request->getPost('academy_cloud_video_duration') == "") {
                ...
                $duration_formatter = explode(':', $this->request->getPost('academy_cloud_video_duration'));
                $hour = sprintf('%02d', $duration_formatter[0]);
                $min = sprintf('%02d', $duration_formatter[1]);
                $sec = sprintf('%02d', $duration_formatter[2]);
                $data['duration'] = $hour . ':' . $min . ':' . $sec;
            ...
            if ($this->request->getPost('html5_video_url_for_mobile_application') == "" || $this->request->getPost('html5_duration_for_mobile_application') == "") {
                ...
                $mobile_app_lesson_url = $this->request->getPost('html5_video_url_for_mobile_application');
                $mobile_app_lesson_duration = $this->request->getPost('html5_duration_for_mobile_application');
            }
            ...
            $course = $this->db->where('id', $this->request->getPost('course_id'))->get('course')->getRowArray();
            ...
            try {
                ...
                $result = $s3->putObject([
                    'Bucket' => $bucket,
                    'Key' => slugify(get_settings('system_name')) . '/' . slugify($course['title']) . '/' . slugify($this->request->getPost('title')) . '-' . time() . '.' . $ext, // Desired file name on Wasabi
                ...
                $duration_formatter = explode(':', $this->request->getPost('wasabi_duration'));
                $hour = sprintf('%02d', $duration_formatter[0]);
                $min = sprintf('%02d', $duration_formatter[1]);
                $sec = sprintf('%02d', $duration_formatter[2]);
                $data['duration'] = $hour . ':' . $min . ':' . $sec;
            ...
            if ($attachment_type == 'audio') {
                ...
                if ($this->request->getPost('system_audio_file_duration') == "") {
                    return json_encode(['error' => get_phrase('invalid_lesson_duration')]);
                }
                ...
                $duration_formatter = explode(':', $this->request->getPost('system_audio_file_duration'));
                $hour = sprintf('%02d', $duration_formatter[0]);
                $min = sprintf('%02d', $duration_formatter[1]);
                $sec = sprintf('%02d', $duration_formatter[2]);
                $data['duration'] = $hour . ':' . $min . ':' . $sec;
        ...
        $data['date_added'] = strtotime(date('D, d-M-Y'));
        $data['summary'] = htmlspecialchars_(remove_js($this->request->getPost('summary', false)));
        $data['is_free'] = htmlspecialchars_($this->request->getPost('free_lesson') ?? "");
        ...
        $this->session->setFlashdata('flash_message', get_phrase('lesson_has_been_added_successfully'));
    ...
    public function edit_lesson($lesson_id)
    {
        ...
        $previous_data = $this->db->table('lesson')->where(array('id' => $lesson_id))->get()->getRowArray();
        ...
        if ($lesson_type == 'video') {
            ...
            if ($attachment_type == 'audio') {
                if (isset($_FILES['system_video_file']) && !empty($_FILES['system_audio_file']['name'])) {
                    ...
                    $previews_audio_url = $this->db->table('lesson')->where(array('id' => $lesson_id))->get()->getRow()->audio_url;
                ...
                if (isset($_FILES['system_video_file']) && !empty($_FILES['system_video_file']['name'])) {
                    ...
                    $previews_video_url = $this->db->table('lesson')->where(array('id' => $lesson_id))->get()->getRow()->video_url;
        ...
        $this->session->setFlashdata('flash_message', get_phrase('lesson_has_been_updated_successfully'));
        return json_encode(['reload' => true]);
    }
    ...
    public function delete_lesson($lesson_id)
    {
        ...
        $watch_history = $this->db->table('watch_histories')->where(array('student_id' => $user_id, 'course_id' => $previous_data['course_id']))->get();
        if ($watch_history->getNumRows() > 0) {
            $data = array();
            if ($watch_history->getRow()->watching_lesson_id == $lesson_id) {
                $data['watching_lesson_id'] = null;
            }

            $completed_lesson_arr = json_decode($watch_history->getRow()->completed_lesson, true);
    ...
    {
        $data['value'] = html_escape($this->request->getPost('banner_title'));
        $this->db->where('key', 'banner_title');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('banner_sub_title'));
        $this->db->where('key', 'banner_sub_title');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('cookie_status'));
        $this->db->where('key', 'cookie_status');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = $this->request->getPost('cookie_note', false);
        $this->db->where('key', 'cookie_note');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = $this->request->getPost('cookie_policy', false);
        $this->db->where('key', 'cookie_policy');
        $this->db->table('frontend_settings')->update($data);



        $data['value'] = $this->request->getPost('facebook');
        $this->db->where('key', 'facebook');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = $this->request->getPost('twitter');
        $this->db->where('key', 'twitter');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = $this->request->getPost('linkedin');
        $this->db->where('key', 'linkedin');
        $this->db->table('frontend_settings')->update($data);


        $data['value'] = $this->request->getPost('about_us', false);
        $this->db->where('key', 'about_us');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = $this->request->getPost('terms_and_condition', false);
        $this->db->where('key', 'terms_and_condition');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = $this->request->getPost('privacy_policy', false);
        $this->db->where('key', 'privacy_policy');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = $this->request->getPost('refund_policy', false);
    ...
    public function update_water_mark()
    {
        ...
        $data['value'] = $this->request->getPost('water_mark_status');
        $this->db->where('key', 'water_mark_status');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = $this->request->getPost('water_mark_opacity');
        $this->db->where('key', 'water_mark_opacity');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = $this->request->getPost('water_mark_speed');
        $this->db->where('key', 'water_mark_speed');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = $this->request->getPost('water_mark_color');
        $this->db->where('key', 'water_mark_color');
        $this->db->table('frontend_settings')->update($data);


        $watermark_type = $this->request->getPost('water_mark_type'); 
    
        if ($watermark_type == 'text') {
            $data['value'] = $this->request->getPost('water_mark', false);
    ...
    public function review_update(){
        ...
        $this->db->where('id', $id);
        $this->db->table('rating')->update($data);
    }
    ...
    {
        $value = html_escape($this->request->getPost('recaptcha_status'));
        ...
        $data['value'] = html_escape($this->request->getPost('recaptcha_sitekey'));
        $this->db->where('key', 'recaptcha_sitekey');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('recaptcha_secretkey'));
        $this->db->where('key', 'recaptcha_secretkey');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('recaptcha_sitekey_v3'));
        $this->db->where('key', 'recaptcha_sitekey_v3');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = html_escape($this->request->getPost('recaptcha_secretkey_v3'));
    ...
    public function handleWishList($course_id)
    {
        ...
        $this->db->table('users')->update($updater);
        return $response;
    }
    ...
    public function getWishLists($user_id = "")
    {
        ...
        $user_details = $this->user_model->get_user($user_id)->getRowArray();
    ...
    public function enrol_student($enrol_user_id, $payer_user_id = "")
    {
        ...
        $purchased_courses = $this->session->get('cart_items');
        foreach ($purchased_courses as $purchased_course) {
            $course_details = $this->get_course_by_id($purchased_course)->getRowArray();
            ...
            if ($this->db->table('enrol')->where(['user_id' => $enrol_user_id, 'course_id' => $purchased_course])->get()->getNumRows() == 0) {
                if ($payer_user_id) {
    ...
    {
        $courses_id = $this->request->getPost('course_id');
        ...
        $this->session->setFlashdata('flash_message', get_phrase('student_has_been_enrolled'));
    }
    ...
    public function shortcut_enrol_a_student_manually()
    {
        ...
        if ($this->db->table('enrol')->where(['course_id' => $course_id, 'user_id' => $user_id])->get()->getNumRows() > 0) {
            $data['gifted_by'] = 0;
            $data['last_modified'] = strtotime(date('D, d-M-Y'));
            $this->db->where('user_id', $user_id);
            $this->db->where('course_id', $course_id);
        ...
        $this->session->setFlashdata('flash_message', get_phrase('student_has_been_enrolled_to_that_course'));
        $response['status'] = 1;
        return json_encode($response);
    }
    ...
    public function enrol_to_free_course($course_id = "", $user_id = "")
    {
        ...
        if ($course_details['is_free_course'] == 1) {
            ...
            $this->session->setFlashdata('flash_message', get_phrase('successfully_enrolled'));
        } else {
            $this->session->setFlashdata('error_message', get_phrase('this_course_is_not_free_at_all'));
    ...
    {
        $purchased_courses = $this->session->get('cart_items');
        $applied_coupon = $this->session->get('applied_coupon');
        ...
        foreach ($purchased_courses as $purchased_course) {
            ...
            if ($course_details['discount_flag'] == 1) {
                ...
                if (addon_status('affiliate_course')  && $this->session->get('course_referee') != "" && $this->session->get('course_reffer_id')) {
            ...
            if ($applied_coupon) {
                $coupon_details = $this->get_coupon_details_by_code($applied_coupon)->getRowArray();
            ...
            $data['date_added'] = strtotime(date('D, d-M-Y'));
            $this->db->table('payment')->insert($data);

            // course_addon start
            if (addon_status('affiliate_course')) :
                if ($this->session->get('course_referee') != "" && $this->session->get('course_reffer_id')) {
                    $CI    = &get_instance();
                    $CI->load->model('addons/affiliate_course_model');
                    $last_affiliate_course_entry = $this->affiliate_course_model->last_affiliate_course_entry($data['user_id'], $this->session->get('course_reffer_id'));
                    $reffre_details_for_aff_table = $this->affiliate_course_model->get_user_by_their_unique_identifier($this->session->get('course_referee'));
                    ...
                    $course_affiliation['course_id'] = $this->session->get('course_reffer_id');
                    ...
                    $this->db->table('course_affiliation')->insert($course_affiliation);
    ...
    public function get_default_lesson($section_id)
    {
        ...
        $this->db->limit(1);
        $this->db->where('section_id', $section_id);
        return $this->db->table('lesson')->get();
    }
    ...
    //     $this->db->where('course_id', $course_id);
    //     $query = $this->db->table('lesson')->get()->getResultArray();
...
public function get_total_duration_of_lesson_by_course_id($course_id)
{
    $query = $this->db->table('lesson')
        ->select("TIME_FORMAT(SEC_TO_TIME(SUM(TIME_TO_SEC(duration))), '%H:%i:%s') AS timeSum", false)
        ->where('course_id', $course_id)
        ->get()
        ->getRowArray();

    return isset($query['timeSum']) && $query['timeSum'] !== null
        ? $query['timeSum'] . ' ' . get_phrase('hours')
        : '0 ' . get_phrase('hours');
}
    ...
    {
        if ($this->db->table('rating')->where(array('user_id' => $data['user_id'], 'ratable_id' => $data['ratable_id'], 'ratable_type' => $data['ratable_type']))->get()->getNumRows() == 0) {
            $this->db->table('rating')->insert($data);
            return 'added';
        } else {
    ...
    {
        $reviews = $this->db->table('rating')->where(array('ratable_type' => $ratable_type, 'user_id' => $this->session->get('user_id')->get(), 'ratable_id' => $ratable_id));
        if ($reviews->getNumRows() > 0) {
            return $reviews->getRowArray();
        } else {
            return array('rating' => 0);
        }
    } 
	public function get_ratings($ratableType = "", $ratableId = "", $isSum = false)
	{
    ...
    {
        $number_of_user_rated = $this->db->table('rating')->where(array(
            'ratable_type' => $ratable_type,
            'ratable_id'   => $ratable_id
        ))->get()->getNumRows();

        $number_of_user_rated_the_specific_rating = $this->db->table('rating')->where(array(
            'ratable_type' => $ratable_type,
            'ratable_id'   => $ratable_id,
            'rating'       => $rating
        ))->get()->getNumRows();
    ...
    function send_new_private_message()
    {
        ...
        $sender     = $this->session->get('user_id');

        //check if the thread between those 2 users exists, if not create new thread
        $num1 = $this->db->table('message_thread')->where(array('sender' => $sender, 'receiver' => $receiver))->get()->getNumRows();
        $num2 = $this->db->table('message_thread')->where(array('sender' => $receiver, 'receiver' => $sender))->get()->getNumRows();
        if ($num1 == 0 && $num2 == 0) {
        ...
        if ($num1 > 0)
            $message_thread_code = $this->db->table('message_thread')->where(array('sender' => $sender, 'receiver' => $receiver))->get()->getRow()->message_thread_code;
        if ($num2 > 0)
            $message_thread_code = $this->db->table('message_thread')->where(array('sender' => $receiver, 'receiver' => $sender))->get()->getRow()->message_thread_code;
    ...
    function send_reply_message($message_thread_code)
    {
        ...
        $message    = html_escape($this->request->getPost('message'));
        $timestamp  = strtotime(date("Y-m-d H:i:s"));
        $sender     = $this->session->get('user_id');

        $message_thread = $this->db->table('message_thread')->where(array('message_thread_code' => $message_thread_code))->get()->getRowArray();
    ...
    {
        return $this->db->table('currency')->get()->getResultArray();
    }
    ...
    function filter_course($search_string = "", $selected_category_id = "", $selected_price = "", $selected_level = "", $selected_language = "", $selected_rating = "", $selected_sorting = "", $per_page = "", $uri_segment = "")
    {
        ...
        if ($selected_category_id != "all") {
            $category_details = $this->get_category_details_by_id($selected_category_id)->getRowArray();
        ...
        if ($selected_rating != "all") {
            ...
            if ($search_string != "") {
                ...
                $this->db->orLike('c.short_description', $search_string);
                $this->db->orLike('c.description', $search_string);
                $this->db->orLike('c.outcomes', $search_string);
                $this->db->orLike('c.language', $search_string);
                $this->db->orLike('c.requirements', $search_string);
                $this->db->orLike('c.meta_keywords', $search_string);
                $this->db->orLike('c.meta_description', $search_string);
                $this->db->groupEnd();
            }
            ...
            if ($scorm_addon_status) {
                $this->db->orWhere('c.course_type', 'scorm');
            }
            if ($h5p_addon_status) {
                $this->db->orWhere('c.course_type', 'h5p');
            }
            $this->db->groupEnd();
            ...
            $this->db->orderBy('avg_rating', 'desc');
            $courses = $this->db->get()->getResultArray();
        ...
        if ($selected_rating != "all") {
            $this->db->groupStart();
            $this->db->whereIn('c.id', $course_ids);
            $this->db->groupEnd();
        }
        ...
        if ($selected_sorting != "" && $selected_sorting == 'newest') {
            ...
            $this->db->orderBy('is_free_course ASC, price ASC, discount_flag DESC, discounted_price ASC');
        } elseif ($selected_sorting != "" && $selected_sorting == 'highest-price') {
            $this->db->orderBy('price DESC, discount_flag ASC, discounted_price DESC');
        } elseif ($selected_sorting != "" && $selected_sorting == 'discounted') {
            $this->db->orderBy('discount_flag DESC, price ASC');
    ...
    public function get_courses($category_id = "", $sub_category_id = "", $instructor_id = 0)
    {
        if ($category_id > 0 && $sub_category_id > 0 && $instructor_id > 0) {
            ...
            $this->db->orLike('user_id', $instructor_id . ',');
            $this->db->orWhere('creator', $instructor_id);
            $this->db->groupEnd();
            ...
            return $this->db->table('course')->where(array('category_id' => $category_id, 'sub_category_id' => $sub_category_id))->get();
        } else {
            return $this->db->table('course')->get();
        }
    }
    ...
    public function sort_section($section_json)
    {
        ...
        foreach ($sections as $key => $value) {
            ...
            $this->db->table('section')->update($updater);
        }
    }
    ...
    public function sort_lesson($lesson_json)
    {
        ...
        foreach ($lessons as $key => $value) {
            ...
            $this->db->where('id', $value);
            $this->db->table('lesson')->update($updater);
        }
    }
    public function sort_question($question_json)
    {
        ...
        foreach ($questions as $key => $value) {
            ...
            $this->db->where('id', $value);
            $this->db->table('question')->update($updater);
        }
    }
    ...
    public function add_quiz($course_id = "")
    {
        ...
        $data['duration'] = $this->request->getPost('quiz_duration');
        $data['attachment_type'] = 'json';

        $attachment_data = array(
            'total_marks' => htmlspecialchars_($this->request->getPost('total_marks')),
            'pass_mark' => $this->request->getPost('pass_mark'),
            'drip_content_for_passing_rule' => $this->request->getPost('drip_content_for_passing_rule')
        );
        $data['attachment'] = json_encode($attachment_data);
        $data['date_added'] = strtotime(date('D, d-M-Y'));
        $data['summary'] = html_escape($this->request->getPost('summary'));
        $data['quiz_attempt'] = html_escape($this->request->getPost('number_of_quiz_retakes'));
        $this->db->table('lesson')->insert($data);
    }
    ...
    public function get_quiz_questions($quiz_id)
    {
        ...
        return $this->db->table('question')->get();
    }
    ...
    function manage_mcq_choice_question($quiz_id, $question_id, $action)
    {
        ...
        if (is_array($this->request->getPost('correct_answers')) && sizeof($this->request->getPost('correct_answers')) == 0) {
            $correct_answers = [""];
        } elseif (!empty($this->request->getPost('correct_answers'))) {
        ...
        $data['title']              = htmlspecialchars_($this->request->getPost('title', false));
        $data['number_of_options']  = htmlspecialchars_($this->request->getPost('number_of_options'));
        $data['type']               = htmlspecialchars_($this->request->getPost('question_type'));
        ...
        if ($action == 'add') {
            ...
            $this->db->where('id', $question_id);
            $this->db->table('question')->update($data);
        }
        return true;
    }
    ...
    function manage_fill_in_the_blank_question($quiz_id, $question_id, $action)
    {
        ...
        $data['title']              = htmlspecialchars_($this->request->getPost('title', false));
        $data['type']               = htmlspecialchars_($this->request->getPost('question_type'));
        $data['correct_answers']    = json_encode(explode(",", $this->request->getPost('correct_answers')));
    ...
    {
        $category_details = $this->get_category_details_by_id($category_id)->getRowArray();
    ...
    function update_watch_history_manually($lesson_id = "", $course_id = "", $user_id = "")
    {
        ...
        $query = $this->db->table('watch_histories')->where(array('course_id' => $course_id, 'student_id' => $user_id))->get();
        $course_progress = $query->getRow()->course_progress;
        if ($query->getNumRows() > 0) {
            $lesson_ids = json_decode($query->getRow()->completed_lesson, true);
            if (!is_array($lesson_ids)) $lesson_ids = array();
            if (!in_array($lesson_id, $lesson_ids)) {
                array_push($lesson_ids, $lesson_id);
                $total_lesson = $this->db->table('lesson')->where(array('course_id' => $course_id))->get()->getNumRows();
                $course_progress = (100 / $total_lesson) * count($lesson_ids);

                if ($course_progress >= 100 && $query->getRow()->completed_date == null) {
                    $completed_date = time();
                } else {
                    $completed_date = $query->getRow()->completed_date;
                }

                $this->db->where('watch_history_id', $query->getRow()->watch_history_id);
                $this->db->table('watch_histories')->update(array('course_progress' => $course_progress, 'completed_lesson' => json_encode($lesson_ids), 'completed_date' => $completed_date, 'date_updated' => time()));
                $is_completed = 1;
            } else {
            ...
            $this->db->table('watch_histories')->insert($insert_data);
        }
    ...
    function enrol_to_free_course_mobile($course_id = "", $user_id = "")
    {
        ...
        if ($this->db->table('course')->where(array('id' => $course_id))->get()->getRow()->is_free_course == 1) :
            $data['gifted_by'] = 0;
            if ($this->db->table('enrol')->where(['course_id' => $course_id, 'user_id' => $user_id])->get()->getNumRows() > 0) {
                $data['course_id'] = $course_id;
                $data['user_id']   = $user_id;
    ...
    {
        return $this->db->table('enrol')->where(array('course_id' => $course_id, 'user_id' => $user_id))->get()->getNumRows();
    }
    ...
    public function get_payouts($id = "", $type = "")
    {
        ...
        return $this->db->table('payout')->get();
    }
    ...
    public function get_total_payout_amount($id = "")
    {
        ...
        $this->db->orderBy('id', 'DESC');
        $payouts = $this->db->table('payout')->where($checker)->get()->getResultArray();
        $total_amount = 0;
    ...
    public function get_requested_withdrawal_amount($id = "")
    {
        ...
        $payouts = $this->db->table('payout')->where($checker)->get();
        if ($payouts->getNumRows() > 0) {
            $payouts = $payouts->getRowArray();
    ...
    public function add_withdrawal_request()
    {
        ...
        if ($total_pending_amount > 0 && $total_pending_amount >= $requested_withdrawal_amount) {
            ...
            $this->session->setFlashdata('flash_message', get_phrase('withdrawal_requested'));
        } else {
            $this->session->setFlashdata('error_message', get_phrase('invalid_withdrawal_amount'));
        }
    }
    ...
    public function delete_withdrawal_request()
    {
        ...
        if ($requested_withdrawal->getNumRows() > 0) {
            ...
            $this->session->setFlashdata('flash_message', get_phrase('withdrawal_deleted'));
        } else {
            $this->session->setFlashdata('error_message', get_phrase('withdrawal_not_found'));
        }
    }
    ...
    public function check_duplicate_payment_for_stripe($transaction_id = "", $stripe_session_id = "", $user_id = "")
    {
        ...
        $query = $this->db->table('payment')->where(array('user_id' => $user_id, 'transaction_id' => $transaction_id, 'session_id' => $stripe_session_id))->get();
        if ($query->getNumRows() > 0) {
            return true;
        } else {
            return false;
    ...
    public function get_coupons($id = null)
    {
        ...
        return $this->db->table('coupons')->get();
    }
    ...
    public function add_coupon()
    {
        if (isset($_POST['code']) && !empty($_POST['code']) && isset($_POST['discount_percentage']) && !empty($_POST['discount_percentage']) && isset($_POST['expiry_date']) && !empty($_POST['expiry_date'])) {
            ...
            $data['discount_percentage'] = $this->request->getPost('discount_percentage') > 0 ? $this->request->getPost('discount_percentage') : 0;
            $data['expiry_date'] = strtotime($this->request->getPost('expiry_date'));
            $data['created_at'] = strtotime(date('D, d-M-Y'));

            $availability = $this->db->table('coupons')->where(['code' => $data['code']])->get()->getNumRows();
            if ($availability) {
                return false;
            } else {
    ...
    public function edit_coupon($coupon_id)
    {
        if (isset($_POST['code']) && !empty($_POST['code']) && isset($_POST['discount_percentage']) && !empty($_POST['discount_percentage']) && isset($_POST['expiry_date']) && !empty($_POST['expiry_date'])) {
            ...
            $availability = $this->db->table('coupons')->get()->getNumRows();
            if ($availability) {
                return false;
            } else {
                $this->db->where('id', $coupon_id);
                $this->db->table('coupons')->update($data);
                return true;
            }
        } else {
            return false;
    ...
    public function check_coupon_validity($coupon_code)
    {
        ...
        $result = $this->db->table('coupons')->get();
    ...
    {
        $total_price  = 0;
        foreach ($this->session->get('cart_items') as $cart_item) {
            $course_details = $this->crud_model->get_course_by_id($cart_item)->getRowArray();
            if ($course_details['discount_flag'] == 1) {
        ...
        if ($this->check_coupon_validity($coupon_code)) {
            $coupon_details = $this->get_coupon_details_by_code($coupon_code)->getRowArray();
    ...
    function get_watch_histories($user_id = 0, $course_id = 0)
    {
        ...
        return $this->db->table('watch_histories')->get();
    }
    ...
    function update_last_played_lesson($course_id = "", $lesson_id = "")
    {
        ...
        if ($course_id != "") {
            if ($query->getNumRows() > 0) {
                $this->db->where('watch_history_id', $query->getRow()->watch_history_id);
                $this->db->table('watch_histories')->update(array('watching_lesson_id' => $lesson_id, 'date_updated' => time()));
            } else {
                if ($lesson_id == "") {
                    $this->db->where('course_id', $course_id);
                    $this->db->orderBy('order', 'asc');
                    ...
                    $this->db->limit(1);
                    $lesson_id = $this->db->table('lesson')->get()->getRow()->id;
                }
                if ($lesson_id > 0) :
                    $data['course_id'] = $course_id;
            ...
            return $query->getRow()->watching_lesson_id;
        }
    }
    ...
    function update_watch_history_with_duration()
    {
        ...
        $course_progress = 0;
        $is_completed = 0;
        $data['watched_course_id'] = htmlspecialchars_($this->request->getPost('course_id'));
        $data['watched_lesson_id'] = htmlspecialchars_($this->request->getPost('lesson_id'));
        $data['watched_student_id'] = $user_id;

        $current_duration = htmlspecialchars_($this->request->getPost('current_duration'));

        $current_history = $this->db->table('watched_duration')->where($data)->get();
        $course_details = $this->db->table('course')->where(['id' => $data['watched_course_id']])->get()->getRowArray();


        if ($current_history->getNumRows() > 0) {
            $current_history = $current_history->getRowArray();
            ...
            $this->db->table('watched_duration')->update(array('watched_counter' => $watched_duration_json, 'current_duration' => $current_duration));
        } else {
            ...
            $this->db->table('watched_duration')->insert($data);
        }
        ...
        $lesson_total_duration = $this->db->table('lesson')->where(array('id' => $data['watched_lesson_id']))->get()->getRow()->duration;
        ...
        if ($is_completed == 1) {
            $query = $this->db->table('watch_histories')->where(array('course_id' => $data['watched_course_id'], 'student_id' => $data['watched_student_id']))->get();
            $course_progress = $query->getRow()->course_progress;
            ...
            if ($query->getNumRows() > 0) {
                ...
                if (!in_array($data['watched_lesson_id'], $lesson_ids)) {
                    ...
                    $total_lesson = $this->db->table('lesson')->where(array('course_id' => $data['watched_course_id']))->get()->getNumRows();
                    $course_progress = (100 / $total_lesson) * count($lesson_ids);
    ...
    function forgot_password()
    {
        ...
        $this->session->setFlashdata('flash_message', get_phrase('check_your_inbox_for_the_request'));
    }
    ...
    function change_password_from_forgot_passord($verification_code = "")
    {
        ...
        $this->db->table('users')->update(array('password' => sha1($this->request->getPost('new_password')), 'verification_code' => $new_verification_code));
        return true;
    }
    ...
    {
        $data['title'] = htmlspecialchars_($this->request->getPost('title'));
        $data['subtitle'] = htmlspecialchars_($this->request->getPost('subtitle'));
        $data['slug'] = slugify($data['title']);
        $data['added_date'] = time();

        $this->db->where('slug', $data['slug']);
        $row = $this->db->table('blog_category')->get();
        if ($row->getNumRows() > 0) {
            return false;
        } else {
            $this->db->table('blog_category')->insert($data);
            return true;
        }
    }
    ...
    function update_blog_category($blog_category_id = "")
    {
        ...
        $this->db->where('slug', $data['slug']);
        $row = $this->db->table('blog_category')->get();
        if ($row->getNumRows() > 0 && $row->getRow()->blog_category_id != $blog_category_id) {
            return false;
        } else {
            $this->db->where('blog_category_id', $blog_category_id);
            $this->db->table('blog_category')->update($data);
            return true;
        }
    }
    ...
    {
        $this->db->where('blog_category_id', $blog_category_id);
        $this->db->table('blog_category')->delete();
    } 
	public function get_blog_categories($blog_category_id = "")
	{
		...
		if ($blog_category_id > 0) {
			$builder->where('blog_category_id', $blog_category_id);
		}
    ...
    function get_blog_category_by_slug($ctaegory_slug = "")
    {
        ...
        return $this->db->table('blog_category')->get();
    }
    ...
    function get_all_blogs($blog_id = "")
    {
        ...
        $this->db->orderBy('blog_id', 'desc');
        return $this->db->table('blogs')->get();
    }
    ...
    function get_blog_comments_by_blog_id($blog_id = "")
    {
        ...
        return $this->db->table('blog_comments')->get();
    }

    function get_blog_comments_by_parent_id($parent_id = "")
    {
        $this->db->where('parent_id', $parent_id);
        $this->db->orderBy('added_date', 'asc');
        return $this->db->table('blog_comments')->get();
    }

    function add_blog_comment($blog_id = "", $user_id = "")
    {
        $data['comment'] = htmlspecialchars_($this->request->getPost('comment'));
    ...
    function update_blog_comment($blog_comment_id = "", $user_id = "")
    {
        ...
        $this->db->table('blog_comments')->update($data);
    }
    function delete_comment($blog_comment_id = "", $user_id = "")
    {
        ...
        $this->db->where('blog_comment_id', $blog_comment_id);
        $this->db->table('blog_comments')->delete();
    }
    ...
    {
        $data['title'] = htmlspecialchars_($this->request->getPost('title'));
        $data['blog_category_id'] = htmlspecialchars_($this->request->getPost('blog_category_id'));
        $data['keywords'] = htmlspecialchars_($this->request->getPost('keywords'));
        $data['description'] = htmlspecialchars_(remove_js($this->request->getPost('description', false)));
        $data['added_date'] = time();
        $data['user_id'] = $this->session->get('user_id');

        if ($this->session->get('admin_login')) {
            $data['is_popular'] = htmlspecialchars_($this->request->getPost('is_popular'));
            $data['status'] = 1;
        } else {
            $data['status'] = 'pending';
            $data['is_popular'] = 0;
    ...
    {
        $blog = $this->get_blogs($blog_id)->getRowArray();
        ...
        $this->db->where('blog_id', $blog_id);
        $this->db->table('blogs')->update($data);
    }

    function update_blog_status($blog_id = "")
    {
        $current_status = $this->get_blogs($blog_id)->getRow()->status;
        if ($current_status == 1) :
            $this->db->where('blog_id', $blog_id);
            $this->db->table('blogs')->update(array('status' => 0));
        else :
            $this->db->where('blog_id', $blog_id);
            $this->db->table('blogs')->update(array('status' => 1));
        endif;
    }
    ...
    {
        $blog = $this->get_blogs()->getRowArray();
        unlink('uploads/blog/banner/' . $blog['banner']);
        unlink('uploads/blog/thumbnail/' . $blog['thumbnail']);

        $this->db->where('blog_id', $blog_id);
        $this->db->table('blogs')->delete();
    ...
    {
        $data['value'] = htmlspecialchars_($this->request->getPost('blog_page_title'));
        $this->db->where('key', 'blog_page_title');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = htmlspecialchars_($this->request->getPost('blog_page_subtitle'));
        $this->db->where('key', 'blog_page_subtitle');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = htmlspecialchars_($this->request->getPost('instructors_blog_permission'));
        $this->db->where('key', 'instructors_blog_permission');
        $this->db->table('frontend_settings')->update($data);

        $data['value'] = htmlspecialchars_($this->request->getPost('blog_visibility_on_the_home_page'));
    ...
    {
        $this->db->where('student_id', $this->session->get('user_id'));
        $this->db->where('course_id', $course_id);
        $query = $this->db->table('watch_histories')->get();

        if ($query->getNumRows() > 0) {
            $previous_result = json_decode($query->getRow()->quiz_result, true);
            if ($quiz_id > 0) {
    ...
    {
        $settings_data['lesson_completion_role'] = htmlspecialchars_($this->request->getPost('lesson_completion_role'));
        $time = htmlspecialchars_($this->request->getPost('minimum_duration'));
        $time = explode(':', $time);
        $seconds = ($time[0] * 3600) + ($time[1] * 60) + $time[2];
        $settings_data['minimum_duration'] = $seconds;
        $settings_data['minimum_percentage'] = htmlspecialchars_($this->request->getPost('minimum_percentage'));
        $settings_data['locked_lesson_message'] = htmlspecialchars_($this->request->getPost('locked_lesson_message', false));
    ...
    {
        $builder = $this->qb('custom_page');

        if ($custom_page_id > 0) {
            $builder->where('custom_page_id', $custom_page_id);
        }

        if ($button_position !== "") {
            $builder->where('button_position', $button_position);
        }
    ...
    {
        $data['page_title'] = htmlspecialchars_($this->request->getPost('page_title'));
        $data['page_content'] = htmlspecialchars_($this->request->getPost('page_content', false));
        $data['page_url'] = slugify($this->request->getPost('page_url'));
        $data['button_title'] = htmlspecialchars_($this->request->getPost('button_title'));
        $data['button_position'] = htmlspecialchars_($this->request->getPost('button_position'));
        $data['status'] = 1;

        $this->db->table('custom_page')->insert($data);
    }
    ...
    function update_custom_page($custom_page_id = "")
    {
        ...
        $this->db->table('custom_page')->update($data);
    }

    function delete_custom_page($custom_page_id = "")
    {
        $this->db->where('custom_page_id', $custom_page_id);
        $this->db->table('custom_page')->delete();
    }
    ...
    function get_notifications($id = "", $status = null)
    {
        ...
        $this->db->orderBy('status ASC, id desc');
        return $this->db->table('notifications')->get();
    }
    ...
    function notification_enable_diable()
    {
        ...
        $notification_setting_row = $this->db->where('id', $id)->get('notification_settings')->getRowArray();
    ...
    function add_newsletter()
    {
        ...
        $this->db->table('newsletters')->insert($data);
    }

    function update_newsletter($id)
    {
        $data['subject'] = $this->request->getPost('subject');
        $data['description'] = $this->request->getPost('description', false);
        $data['updated_at'] = time();

        $this->db->where('id', $id);
        $this->db->table('newsletters')->update($data);
    }

    function delete_newsletter($id)
    {
        $this->db->where('id', $id);
        $this->db->table('newsletters')->delete();
    }
    //End newsletter
    ...
    {
        $faqs = array();
        foreach (array_filter($this->request->getPost('questions')) as $key => $question) {
            $faqs[$key]['question'] = $question;
            $faqs[$key]['answer'] = $this->request->getPost('answers')[$key];
        }
    ...
    function update_motivational_speech()
    {
        $motivations = array();
        $images = array();
        foreach (array_filter($this->request->getPost('titles')) as $key => $title) {
            $motivations[$key]['title'] = $title;
            $motivations[$key]['description'] = $this->request->getPost('descriptions')[$key];
            ...
            if ($_FILES['images']['name'][$key] != "") {
                ...
                $motivations[$key]['image'] = $this->request->getPost('previous_images')[$key];
            }
    ...
    function get_related_courses($parent_category = "", $child_category = "", $current_course_id = "", $limit = "10")
    {
        ...
        if ($child_category != "") {
            $this->db->orWhere('sub_category_id', $child_category);
        }
        $this->db->groupEnd();
    ...
    function course_page_accessibility($course_id = "", $user_id = "")
    {
        ...
        if ($row->getNumRows() > 0) {
            $course_details = $row->getRowArray();
    ...
    function get_course_instructors_id($course_id = "")
    {
        $row = $this->get_course_by_id($course_id);
        if ($row->getNumRows() > 0) {
            $instructors_arr = explode(',', $row->getRow()->user_id);
    ...
    function update_contact_info()
    {
        ...
        $data['office_hours'] = $this->request->getPost('office_hours');
        ...
        if ($row->getNumRows() > 0) {
            ...
            $this->db->table('frontend_settings')->insert(['key' => 'contact_info', 'value' => $contact_information]);
        }
    }
    ...
    function update_custom_codes()
    {
        ...
        $embed_code = $this->request->getPost('embed_code', false);
        ...
        if ($row->getNumRows() > 0) {
            ...
            $this->db->table('frontend_settings')->insert(['key' => 'custom_css', 'value' => $custom_css]);
        }
        ...
        if ($row->getNumRows() > 0) {
            ...
            $this->db->table('frontend_settings')->insert(['key' => 'embed_code', 'value' => $embed_code]);
        }
    }
    ...
    function get_contacts($id = "")
    {
        ...
        return $this->db->table('contact')->get();
    }
    ...
    public function wasabi_file_delete($file_name, $course_id)
    {
        ...
        $course = $this->db->where('id', $course_id)->get('course')->getRowArray();
    ...
    function sendEmailToAssignedAddresses()
    {
        ...
        if ($lastEmailSendingTime->getNumRows() > 0) {
            $lastEmailSendingTime = $lastEmailSendingTime->getRow()->value;
            ...
            $this->db->table('settings')->insert(['key' => 'lastEmailSendingTime', 'value' => time()]);
        }
        ...
        foreach ($email_list->getResultArray() as $row) {
    ...
    function join_bbb_meeting_by_curl_calls($course_id = "", $is_moderator = false)
    {
        ...
        $bbb_meeting_info = $this->db->where('course_id', $course_id)->get('bbb_meetings')->getRowArray();
    ...
    {
        return $this->db->table('seo_fields')->get();
    }
    ...
    function save_seo_settings($route = "")
    {
        ...
        $data['meta_robot']       = html_escape($this->request->getPost('meta_robot'));
        $data['canonical_url']    = html_escape($this->request->getPost('canonical_url'));
        $data['custom_url']       = html_escape($this->request->getPost('custom_url'));
        ...
        if ($_FILES['og_image']['name'] != "") {
            ...
            $data['og_image'] = $this->request->getPost('old_og_image');
        }
        
        $this->db->where('route', $route);
        $this->db->table('seo_fields')->update($data);
    }

    // Fetch SEO data based on the route
    public function get_seo_by_route($route)
    {
        return $this->qb('seo_fields')
            ->where('route', $route)
            ->get()
            ->getRow();
    }  
    ...
    {
        $data['title']          = html_escape($this->request->getPost('title'));
        $data['type']           = html_escape($this->request->getPost('type'));
        $data['condition_from'] = html_escape($this->request->getPost('condition_from'));
        $data['condition_to']   = html_escape($this->request->getPost('condition_to'));
        ...
        $this->db->table('badges')->insert($data);
    }
    
    

    public function badges_List() {
        return $this->db->table('badges')->get();
    }
    
    public function badges_delete($badge_id = '')
    {
        $badge = $this->db->table('badges')->where(['id' => $badge_id])->get()->getRowArray();
    ...
    {
        $badge_id = html_escape($this->request->getPost('badge_id'));
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            // Delete old image
            $old = $this->db->table('badges')->where(['id' => $badge_id])->get()->getRowArray();
        ...
        // Update badge
        $this->db->where('id', $badge_id);
        $this->db->table('badges')->update($data);
    }
