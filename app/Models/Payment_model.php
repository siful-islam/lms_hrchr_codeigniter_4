...
class Payment_model extends NativeBaseModel
{
    ...
    function configure_course_payment()
    {
        ...
        // Course gift
        $data['gift_to_user_id'] = $this->session->get('gift_to_user_id');
    ...
    {
        $payout_request = $this->db->where('user_id', $is_instructor_payout_user_id)->where('status', 0)->get('payout')->getRowArray();
        $amount = $payout_request['amount'];
        $items = array();
        $total_payable_amount = 0;
        $instructor_details = $this->user_model->get_all_user($is_instructor_payout_user_id)->getRowArray();
    ...
    public function check_paypal_payment($identifier = "")
    {
        ...
        $payment_gateway = $this->db->table('payment_gateways')->where(['identifier' => $identifier])->get()->getRowArray();

        if ($payment_details['is_instructor_payout_user_id'] > 0) {
            $instructor_details = $this->user_model->get_all_user($payment_details['is_instructor_payout_user_id'])->getRowArray();
    ...
    {
        /***UN-COMPLETE***/
        $payment_gateway = $this->db->table('payment_gateways')->where(['identifier' => "skrill"])->get()->getRowArray();
    ...
    public function check_payu_payment($identifier = "")
    {
        ...
        $order_id = $this->session->get("payu_order_id");
        try {
            ...
            if ($response->getStatus() === "SUCCESS" && $response->getResponse()->orders[0]->totalAmount) {
                ...
                $this->session->remove('payu_order_id');
                return true;
            }
    ...
    public function check_xendit_payment($identifier = "")
    {
        ...
        $id = $this->session->get("xendit_invoice_id");
        ...
        try {
            ...
            if ($getInvoice["status"] === "PAID" && $getInvoice["amount"] == ($payment_details['total_payable_amount'] * 100)) {
                ...
                $this->session->remove('xendit_invoice_id');
                return true;
            }
        } catch (\Exception $exception) {
    ...
    function checkLogin($payment_info = "")
    {
        if ($this->session->get('user_id') > 0) {
            return $this->session->get('payment_details');
        } else {
            $cart_items = array();
            ...
            if ($query->getNumRows() > 0) {
                ...
                $this->session->set('cart_items', $cart_items);
                $this->session->set('applied_coupon', $payment_info[2]);
                $this->session->set('payment_details', $payment_details);
                $this->session->set('total_price_of_checking_out', $payment_details['total_payable_amount']);
    ...
    function check_tazapay_payment(){
        ...
        $id = $this->session->get('tazapay_id');

        $payment_details = $this->session->get('payment_details');
        $payment_gateway = $this->db->table('payment_gateways')->where(['identifier' => 'tazapay'])->get()->getRowArray();
        $customer_details = $this->user_model->get_all_user($this->session->get('user_id'))->getRowArray();
        ...
        if ($err) {
            ...
            if(is_array($responseArr) && $responseArr['status'] == 'success'){
                if($responseArr['data']['amount'] == $responseArr['data']['amount_paid']){
                    $this->session->remove('tazapay_id');
                    return true;
                }else{
                    return false;
                }
            }else{
                $this->session->setFlashdata('error_message', get_phrase('An error occurred'));
