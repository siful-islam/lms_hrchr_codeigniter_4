...
class Addon_model extends NativeBaseModel
{
	...
	public function install_addon($param1 = "") {
		...
		if ($param1 == 'install'){
			if($_SERVER['SERVER_NAME'] == 'localhost' || $_SERVER['SERVER_NAME'] == '127.0.0.1'){
				...
				// if($addon_exits->getNumRows() > 0){
				// 	$this->session->setFlashdata('error_message', get_phrase('you_have_already_used_this_purchase_code'));
				// 	redirect(site_url('admin/addon'), 'refresh');
				// }

				$status_response = $this->crud_model->curl_request($purchase_code);
				if(!$status_response){
					$this->session->setFlashdata('error_message', get_phrase('purchase_code_is_wrong').'. '.get_phrase('please_enter_your_valid_purchase_code'));
					redirect(site_url('admin/addon'), 'refresh');
				}
			}
		}
		...
		if (!empty($zipped_file_name)) {
			...
			$addon_details = $this->db->table('addons')->where(array('unique_identifier' => $data['unique_identifier']))->get();

			if ($addon_details->getNumRows() > 0) {
				$data['updated_at'] = strtotime(date('d-m-y'));
				$this->db->where('unique_identifier', $data['unique_identifier']);
				$this->db->table('addons')->update($data);
			...
			if ($param1 == 'install') {
				$this->session->setFlashdata('flash_message', get_phrase('addon_installed_successfully'));
			}else{
				$this->session->setFlashdata('flash_message', get_phrase('addon_updated_successfully'));
			}
			redirect(site_url('admin/addon'), 'refresh');
		}else{
			$this->session->setFlashdata('error_message', get_phrase('no_addon_found'));
			redirect(site_url('admin/addon'), 'refresh');
		}
	...
	public function addon_activate($addon_id = ""){
		$check_addon_status = $this->db->table('addons')->where(array('id' => $addon_id))->get()->getRow()->status;
	...
	public function addon_list($unique_identifier = ""){
		...
		return $this->db->table('addons')->get();
	}
