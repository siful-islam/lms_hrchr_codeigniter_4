...
<div id="quiz-stuffs" style="font-size: 40px;">
    <div id="quiz-body" class="text-center py-4">
        <div class="p-2" id="quiz-header">
            ...
            <?php echo get_phrase("number_of_questions"); ?> : <strong><?php echo count($quiz_questions->getResultArray()); ?></strong><br>
            <?php if (count($quiz_questions->getResultArray()) > 0): ?>
        ...
        <form class="" id = "quiz_form" action="" method="post">
            <?php if (count($quiz_questions->getResultArray()) > 0): ?>
                <?php foreach ($quiz_questions->getResultArray() as $key => $quiz_question):
                    ...
                    <button type="button" name="button" class="btn red mt-2 mb-2" id = "next-btn-<?php echo $quiz_question['id'];?>" style="color: #fff; font-size: 30px;" <?php if(count($quiz_questions->getResultArray()) == $key+1):?>onclick="submitQuiz()"<?php else: ?>onclick="showNextQuestion('<?php echo $key+2; ?>')"<?php endif; ?> disabled><?php echo count($quiz_questions->getResultArray()) == $key+1 ? get_phrase("check_result") : get_phrase("submit_and_next"); ?></button>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </form>
