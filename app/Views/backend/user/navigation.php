...
<div class="left-side-menu left-side-menu-detached">
	...
	<ul class="metismenu side-nav side-nav-light">
		...
		<?php if (get_settings('allow_instructor') == 1) : ?>
			<?php if (session()->get('is_instructor')) : ?>
				...
				<?php if (addon_status('bootcamp') && session()->get('is_instructor') == 1) : ?>
				...
				<?php if (addon_status('tutor_booking') && session()->get('is_instructor')==1) : ?>
					<li class="side-nav-item">
		...
		<?php if (get_frontend_settings('instructors_blog_permission') && session()->get('is_instructor')) : ?>
			<li class="side-nav-item <?php if ($page_name == 'blog' || $page_name == 'blog_add' || $page_name == 'blog_edit' || $page_name == 'instructors_pending_blog') : ?> active <?php endif; ?>">
				...
				<ul class="side-nav-second-level" aria-expanded="false">
					...
					<li class="<?php if ($page_name == 'instructors_pending_blog') echo 'active'; ?>">
						<a href="<?php echo site_url('user/pending_blog'); ?>"><?php echo get_phrase('pending_blog'); ?> <span class="badge badge-danger-lighten"><?php echo $crudModel->get_instructors_pending_blog()->getNumRows(); ?></span></a>
					</li>
				</ul>
			</li>
		<?php endif; ?>
		...
		if (addon_status('affiliate_course') ) :
			...
			$x = $affiliateModel->is_affilator(session()->get('user_id'));
		...
		<li class="side-nav-item">
			<a href="<?php echo site_url('home/my_messages'); ?>" class="side-nav-link">
				...
				<?php
				$db->where('message_thread.receiver', session()->get('user_id'));
				$db->where('message.sender !=', session()->get('user_id'));
				$db->where('message.read_status', 0);
				$db->from('message_thread');
				$db->join('message', 'message_thread.message_thread_code = message.message_thread_code');
				$unreaded_message = $db->get()->getNumRows();
				?>
