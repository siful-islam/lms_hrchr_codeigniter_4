...
$months = array('january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'october', 'november', 'december');
    ...
    for ($i = 0; $i < 12; $i++) {
        ...
        $course_ids = $crudModel->get_instructor_wise_courses(session()->get('user_id'), 'simple_array');
        if (count($course_ids) > 0) {
            $db->selectSum('instructor_revenue');
            $db->where('date_added >=' , strtotime($first_day_of_month));
            $db->where('date_added <=' , strtotime($last_day_of_month));
            $db->whereIn('course_id', $course_ids);
            $total_instructor_revenue = $db->table('payment')->get()->getRow()->instructor_revenue;
        }else{
    ...
    $number_of_active_course  = $crudModel->get_status_wise_courses_for_instructor('active')->getNumRows();
    $number_of_pending_course = $crudModel->get_status_wise_courses_for_instructor('pending')->getNumRows();
?>
