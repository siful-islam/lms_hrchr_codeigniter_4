...
function showAjaxModal(url, header, modalSize, modalScrollable)
{
	var $dialog = jQuery('#scrollable-modal .modal-dialog');
    $dialog.removeClass('modal-sm modal-md modal-lg modal-xl modal-fullscreen');
    if (modalSize) {
        $dialog.addClass('modal-' + modalSize);
    }
    var isScrollable = modalScrollable !== false;
    $dialog.toggleClass('modal-dialog-scrollable', isScrollable);	 
    ...
    $.ajax({
		...
		error: function(xhr, status, error) {
			jQuery('#scrollable-modal .modal-body').html('<div class="alert alert-danger">Error loading content: ' + error + '</div>');
			jQuery('#scrollable-modal .modal-title').html('Error');
		}
    });
}
