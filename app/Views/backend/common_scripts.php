...
$upload_max_filesize_kb = str_replace('m', '', strtolower(ini_get('upload_max_filesize'))) * 1024; ?>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var table = $('#course-datatable-server-side').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "<?php echo site_url(strtolower(session()->get('role')) . '/get_courses') ?>",
                    "dataType": "json",
                    "type": "POST",
                    "data": {
...
<?php
$email_list = $db->where('status', 'pending')->orWhere('status', 'faild')->get('newsletter_histories')->getNumRows();
if ($email_list > 0) :
?>
