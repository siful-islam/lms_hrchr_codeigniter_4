...
foreach($notifications->getResultArray() as $notification): ?>
    <div class="dropdown-item notify-item cursor-pointer <?php if($notification['status'] == 0) echo 'unread' ?>">
        <?php if($notification['type'] == 'signup'): ?>
            <div class="notify-icon">
                <img src="<?php echo $userModel->get_user_image_url($notification['from_user']); ?>" class="img-fluid rounded-circle" alt="User image" />
            </div>
        <?php else: ?>
