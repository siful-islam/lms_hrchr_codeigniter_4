...
echo get_phrase('private_message'); ?></h4>
            </div> <!-- end card body-->
        </div> <!-- end card -->
    </div><!-- end col-->
</div>
        ...
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3">
                ...
                <ul class="navbar-nav">
                    ...
                    $db->where('sender', $current_user);
                    $db->orWhere('receiver', $current_user);
                    ...
                    foreach($message_threads as $row):
                        ...
                        $unread_message_number = $crudModel->count_unread_message_of_thread($row['message_thread_code']);
                        ?>
                        <li class="nav-item">
                            <a class="text-left mb-1 btn btn-light d-block <?php if (isset($current_message_thread_code) && $current_message_thread_code == $row['message_thread_code'])echo 'active';?>" href="<?php echo site_url('admin/message/message_read/' . $row['message_thread_code']);?>">
                                ...
                                <?php
                                    $user_details = $db->table('users')->where(array('id' => $user_to_show_id))->get()->getRowArray();
