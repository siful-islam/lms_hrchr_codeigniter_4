...
if ($selected_category_id == 0) {
    foreach ($categories->getResultArray() as $category) {
        $selected_category_id = $category['id'];
        $sub_categories = $db->table('category')->where(array('parent' => $selected_category_id))->get();
        break;
    }
}else {
    $sub_categories = $db->table('category')->where(array('parent' => $selected_category_id))->get();
}
$selected_category_details = $crudModel->get_categories($selected_category_id)->getRowArray();
?>
<hr />
        ...
        <div class="panel panel-primary" data-collapsed="0">
            <div class="panel-body">
                ...
                <table class="table table-bordered datatable" id="table-1">
                    ...
                    <tbody>
                        <?php foreach ($sub_categories->getResultArray() as $sub_category): ?>
                            <tr class="gradeU">
