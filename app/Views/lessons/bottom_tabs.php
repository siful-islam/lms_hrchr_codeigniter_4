...
if (isset($lesson_details) && is_array($lesson_details) && count($lesson_details) > 0) : ?>
	...
	<?php endif; ?>
<?php if (live_class_status($course_id)) : ?>
	<li class="nav-item" role="presentation">
...
<div class="tab-content ct-tabs-content">
	<?php if (isset($lesson_details) && is_array($lesson_details) && count($lesson_details) > 0) : ?>
		<div class="tab-pane fade" id="summary-class-content" role="tabpanel" aria-labelledby="summary-class-tab">
			...
			<?php $resource_files = $db->orderBy('id', 'desc')->where('lesson_id', $lesson_details['id'])->get('resource_files')->getResultArray(); ?>
	...
	<div class="tab-pane fade" id="live-class-content" role="tabpanel" aria-labelledby="live-class-tab">
		...
		<?php $bbb_meeting = $db->where('course_id', $course_id)->get('bbb_meetings');
		...
		<?php if (addon_status('live-class')) : ?>
			<?php $live_class = $db->table('live_class')->where(array('course_id' => $course_id))->get();
			if ($live_class->getNumRows() > 0) :
				$live_class_scheduled = 1;
				$live_class = $db->table('live_class')->where(array('course_id' => $course_id))->get()->getRowArray(); ?>
				<div class="live_class">
		...
		<?php if (addon_status('jitsi-live-class')) : ?>
			<?php $live_class = $db->table('jitsi_live_class')->where(array('course_id' => $course_id))->get();
			if ($live_class->getNumRows() > 0) :
				$live_class_scheduled = 1;
