...
<body >
    <div id="capture" style="position: relative; width: 750px; margin-left: auto; margin-right: auto;">
        <?php
            ...
            $certificate = $db->table('certificates')->where(array('shareable_url' => $certificate_identifier))->get()->getRowArray();



            $course = $crudModel->get_course_by_id($certificate['course_id'])->getRowArray();

            $student_row = $userModel->get_all_user($certificate['student_id'])->getRowArray();

            $instructor_row = $userModel->get_all_user($course['creator'])->getRowArray();

            $lessons = $crudModel->get_lessons('course', $certificate['course_id']);
            $lesson_count=$lessons->getNumRows();
            $course_duration = $crudModel->get_total_duration_of_lesson_by_course_id($certificate['course_id']);
