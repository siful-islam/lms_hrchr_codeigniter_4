...
class Blog extends BaseController
{
    ...
    function blogs($param1 = ''){
        ...
        if(isset($_GET['search']) && !empty($_GET['search'])){
            ...
            $this->db->orLike('description', $_GET['search']);
            $this->db->where('status', 1);
            $total_rows = $this->db->table('blogs')->get()->getNumRows();
            ...
            $blog_category_id = $this->crud_model->get_blog_category_by_slug($_GET['category'])->getRow()->blog_category_id;
    ...
    function details($blog_slug = "", $blog_id = ""){
        ...
        if($blog_row->getNumRows() == 0){
            $this->session->setFlashdata('error_message', site_phrase('This blog is not available'));
            redirect($_SERVER['HTTP_REFERER'], 'refresh');
        }
        
        $page_data['blog_details'] = $blog_row->getRowArray();
    ...
    function add_blog_comment($blog_id = ""){
        ...
        if($blog_id > 0 && $user_id > 0){
            ...
            $this->session->setFlashdata('flash_message', site_phrase('your_reply_has_been_successfully_published'));
            redirect($_SERVER['HTTP_REFERER'], 'refresh');
        }else{
            $this->session->setFlashdata('error_message', site_phrase('make_sure_you_have_logged_in'));
            redirect($_SERVER['HTTP_REFERER'], 'refresh');
        }
    }
    ...
    function delete_comment($blog_comment_id = "", $blog_id = ""){
        $blog_details = $this->crud_model->get_blogs($blog_id)->getRowArray();
        $user_id = $this->session->get('user_id');
        $this->crud_model->delete_comment($blog_comment_id, $user_id);
        $this->session->setFlashdata('flash_message', site_phrase('your_comment_has_been_deleted_successfully'));
