...
class Payment extends BaseController
{
    ...
    public function __construct()
    {
        ...
        if (!$this->session->get('payment_details') || !$this->session->get('user_id')) {
            $this->session->setFlashdata('error_message', site_phrase('payment_not_configured_yet'));
    ...
    public function create_ssl_commerz_payment()
    {
        ...
        $post_data['cus_name'] = $this->session->get("name");
    ...
    public function create_payu_payment()
    {
        ...
        $user = $this->user_model->get_user($this->session->get("user_id"))->getRow();
        ...
        try {
            ...
            $this->session->set("payu_order_id", $response->getResponse()->orderId);
    ...
    public function create_xendit_payment()
    {
        ...
        try {
            ...
            $id = $createInvoice['id'];
            $this->session->set("xendit_invoice_id", $id);
    ...
    public function create_doku_payment()
    {
        ...
        $order_data = array(
            ...
            'customerName' => $this->session->get('name'),
            'phone' => $user->phone,
    ...
    function aamarpay_payment_link()
    {
        ...
        $payment_details = $this->session->get('payment_details');
        $payment_gateway = $this->db->table('payment_gateways')->where(['identifier' => 'aamarpay'])->get()->getRowArray();
        ...
        $payment_info = array($user_id, $payment_details, $this->session->get('applied_coupon'), microtime(true));
    ...
    function tazapay_payment_form()
    {
        ...
        if ($err) {
            $this->session->setFlashdata('error_message', $err);
            ...
            if(is_array($responseArr) && $responseArr['status'] == 'success'){
                ...
                $this->session->set('tazapay_id', $responseArr['data']['id'], 600);
