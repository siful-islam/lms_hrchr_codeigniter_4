...
class Api extends REST_Controller {
  ...
  public function web_redirect_to_buy_course_get($auth_token = "", $course_id = "", $app_url = ""){
    ...
    if($auth_token != "" && $course_id != "" && is_numeric($course_id)){
      ...
      if ($query->getNumRows() > 0) {
          ...
          $this->session->set('app_url', $app_url.'://');
          $this->session->setFlashdata('flash_message', 'Welcome' . ' ' . $row->first_name . ' ' . $row->last_name);
          ...
          $this->session->set('total_price_of_checking_out', $price);
          $this->session->set('cart_items', $previous_cart_items);

          //redirect to payment page
          redirect(site_url('home/course_payment'), 'refresh');
      } else {
          $this->session->setFlashdata('error_message', 'Invalid auth token');
          redirect(site_url('login'), 'refresh');
      }
    }else{
      $this->session->setFlashdata('error_message', 'Something is wrong');
      redirect(site_url('login'), 'refresh');
    }
  }
  ...
  // $user_sessions = $this->db->table('users')->get()->getRow()->session_id;
    ...
    if (isset($_POST['auth_token']) && !empty($_POST['auth_token'])) {
      ...
      if ($logged_in_user_details['user_id'] > 0) {
        if (isset($_FILES['file']) && $_FILES['file']['name'] != "") {
          $user_image = $this->db->table('users')->where(array('id' => $logged_in_user_details['user_id'])->get())->getRow()->image.'.jpg';
  ...
  public function enroll_free_course_get(){
    if (isset($_GET['auth_token']) && !empty($_GET['auth_token'])) {
      ...
      if ($course_details['is_free_course'] == 1) {
          ...
          if ($this->db->table('enrol')->where(['course_id' => $course_id, 'user_id' => $logged_in_user_details['user_id']])->get()->getNumRows() > 0) {
  ...
  function zoom_live_class_get(){
    ...
    if ($course_details['user_id'] != $user_details['user_id']) {
        $enrolled_history = $this->db->table('enrol')->where(array('user_id' => $user_details['user_id'], 'course_id' => $course_id)->get())->getNumRows();
    ...
    if($access && $course_id > 0){
      $live_class = $this->db->table('live_class')->where(array('course_id' => $course_id)->get());
      if($live_class->getNumRows() > 0){
        $response['zoom_live_class_details'] = $live_class->getRowArray();
      }else{
...
// This is the function for rendering quiz web view for mobile
  ...
  function account_disable_post($auth_token = ""){
    $response = array();
      ...
      if(isset($_POST) && count($_POST)){
          $logged_in_user_details = json_decode($this->token_data_get($auth_token), true);
          $user_details = $this->user_model->get_all_user($logged_in_user_details['user_id'])->getRowArray();
 ...
 public function blogs_get(){
	$blogs = $this->api_model->get_all_blogs();
