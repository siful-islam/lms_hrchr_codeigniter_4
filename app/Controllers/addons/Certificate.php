...
class Certificate extends BaseController
{
    ...
    function position($param1 = ""){
        $certificate_text_positions = htmlspecialchars($this->request->getPost('text_positions', false));
        if($param1 == 'save'){
            $this->db->where('key', 'certificate-text-positons');
            $row = $this->db->table('settings')->get();
            if($row->getNumRows() > 0){
                $this->db->where('key', 'certificate-text-positons');
                $this->db->table('settings')->update(array('value' => $certificate_text_positions));
            }else{
                $this->db->table('settings')->insert(array('key' => 'certificate-text-positons','value' => $certificate_text_positions));
            }
            return true;
        }else{
    ...
    public function download($certificate_identifier = "") {
        if ( !empty($certificate_identifier) && file_exists('uploads/certificates/'.$certificate_identifier.'.jpg')) {
            ...
            $this->session->setFlashdata('flash_message', get_phrase('downloaded'));
            redirect(site_url('home' . $message_thread_code), 'refresh');
        }else{
            $this->session->setFlashdata('error_message', get_phrase('invalid_certificate'));
            redirect(site_url('home' . $message_thread_code), 'refresh');
        }

    }
	public function increase_download_count() {
		$watch_student_id = $this->request->getPost('watch_student_id');
		$watch_course_id = $this->request->getPost('watch_course_id');

		$this->db->set('downloaded_number', 'downloaded_number+1', FALSE);
		$this->db->where('course_id', $watch_course_id);
		$this->db->where('student_id', $watch_student_id);
		$this->db->update('watch_histories');
