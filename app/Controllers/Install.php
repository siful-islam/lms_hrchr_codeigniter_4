...
class Install extends BaseController {
  ...
  function finalizing_setup($param1 = '', $param2 = '') {
    ...
    if ($param1 == 'setup_admin') {
     // $admin_data['unique_identifier'] = '1j7hdmsw8d';
      $admin_data['first_name']       = html_escape($this->request->getPost('first_name'));
      $admin_data['last_name']       = html_escape($this->request->getPost('last_name'));
      $admin_data['email']      = html_escape($this->request->getPost('email'));
      $admin_data['password']   = sha1($this->request->getPost('password'));
      $social_links = array(
          'facebook' => "",
          'twitter'  => "",
          'linkedin' => ""
      ...
      $data['value']  = $this->request->getPost('system_name');
  ...
  function success($param1 = '') {
    ...
    $this->load->database();
    $admin_email = $this->db->table('users')->where(array('id' => 1)->get())->getRow()->email;
