...
class Updater extends BaseController
{
    ...
    function update($task = '', $purchase_code = '')
    {
        ...
        if (strval($json['require_version']) != strval(get_settings('version'))){
            $this->session->setFlashdata('error_message', get_phrase('it_looks_like_you_are_skipping_a_version').'. '.get_phrase('please_update_version').' '.$json['require_version'].' '.get_phrase('first'));
            redirect(site_url('admin/system_settings'), 'refresh');
        }
        ...
        $this->session->setFlashdata('flash_message', get_phrase('product_updated_successfully'));
