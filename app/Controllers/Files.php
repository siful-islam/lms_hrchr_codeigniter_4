...
class Files extends BaseController
{
    ...
    function index()
    {
        ...
        if (isset($_GET['course_id']) && isset($_GET['lesson_id']) && $user_id > 0) {
            ...
            $multi_instructors = explode(',', $this->crud_model->get_course_by_id($course_id)->getRow()->user_id);
            $lesson = $this->crud_model->get_lessons('lesson', $lesson_id)->getRowArray();
            $get_lesson_type = get_lesson_type($lesson_id);

            if (enroll_status($course_id) == 'valid' || $this->session->get('admin_login') == '1' || in_array($user_id, $multi_instructors)) {
